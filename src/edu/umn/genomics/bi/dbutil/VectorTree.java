/*
 * @(#) $RCSfile: VectorTree.java,v $ $Revision: 1.2 $ $Date: 2002/07/30 19:44:46 $ $Name: TableView1_2 $
 *
 ...
 * that can be used for data caching.
 * Branches in tree can be designated to be SoftReferences to Vectors 
 * thus providing a cache that can be dumped when memory limits occur.
 * 
 * @author       J Johnson
 * @version $Revision: 1.2 $ $Date: 2002/07/30 19:44:46 $  $Name: TableView1_2 $ 
 * @since        1.0
 */
public class VectorTree {
  /*
  rowTree for branchSize = 2, size = 8  
   pow    2 1 0    data vector
   ---    - - -    -----------
          0
            0
              0 -> row vector
  ...
  */
   ...
   * @param size  The number of leaf entries.
   ...
   * @param o The Object representing this node, either a Vector or a SoftReference.
   * @return The Vector for this node.
   */
  ...
  synchronized public void setSize(int size) {
    ...
    if (newDepth > oldDepth) { // Move current root to a lower level branch
      ...
      v.setElementAt(getNode(oldroot,branchCount,d >= minRef && d <= maxRef),0); 
  ...
  /** 
   * Get the Vector element for the given index.
   * The index ranges from 0 to the size - 1.
   * @param index The element Vector to return.
   * @return The Vector for the given index, it will be empty if this 
