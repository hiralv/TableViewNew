/*
 * @(#) $RCSfile: DBTable.java,v $ $Revision: 1.3 $ $Date: 2003/10/03 12:50:45 $ $Name: TableView1_2 $
 *
 ...
 * @author       J Johnson
 * @version $Revision: 1.3 $ $Date: 2003/10/03 12:50:45 $  $Name: TableView1_2 $ 
 * @since        1.0
...
public class DBTable implements Comparable {
   ...
   * @param table the name pattern for the table name.
   * @param types the database table types to include.
   * @return a list of DBTable ojbects that describe tables matching the arguments.
   * @see java.sql.DatabaseMetaData#getColumns(String, String, String, String)
   * @throws NullPointerException if dbmd is null
  ...
  public DBColumn getColumn(String name) {
    ...
    for (int i = 0; i < getColumnCount(); i++) {
      if (columns[i].getColumnName().equals(name) || columns[i].getQualifiedName().equals(name)) {
        return columns[i];
      }
    }
    return null;
