/*
 * @(#) $RCSfile: GraphDataModel.java,v $ $Revision: 1.5 $ $Date: 2003/07/28 15:07:09 $ $Name: TableView1_2 $
 *
 ...
 * format "XY"
 * @author       J Johnson
 * @version $Revision: 1.5 $ $Date: 2003/07/28 15:07:09 $  $Name: TableView1_2 $ 
 * @since        1.0
 */
public class GraphDataModel extends AbstractDataModel {
   ...
   * @param dataArray an array of data values.
   */
  ...
  /**
   * Set the data values to be displayed by on the graph.
   * @param dataArray an array of x and y values
   * @see GraphDataModel#FORMAT_Y
   */
  ...
  /** 
   * Transform an array of any primitive number type, any Number instances, 
   * or String instances parseable as numbers into an array of double.
   * @param dataArray an array of values
  ...
  public static double[] doubleArrayFromObject(Object dataArray) {
    ...
    try {
      if (dataArray.getClass().isArray()) {
        ...
        } else if (Class.forName("java.lang.String").isAssignableFrom(compClass)) {
            ...
            if (pp.getIndex() < ns.length()) {
              throw new IllegalArgumentException(
                 "GraphDataModel.setData(dataArray) " + 
                  "unable to parse element " +  i + " :  " + sa[i] + 
                  "  as a number");
            }
            ds[i] = n.doubleValue(); 
          }
    ...
    if (FORMAT_Y.equalsIgnoreCase(format)) {
      ...
      for (int i = 0,di = 0; i < np; i++, di+=stride) {
        for (int c = 0; c < fmt.length(); c++) {
          ...
          switch (ch) {
          case 'X':
//System.err.println("X" + i + " " + d + " " + c + " " + di + " : " + dataset[di+c] + " " + axes[0].getIntPosition(dataset[di+c]) + "   " + ( x + axes[0].getIntPosition(dataset[di+c])) + " @ " + x);
            pnts[d++][i] = x + axes[0].getIntPosition(dataset[di+c]);
            break;
          case 'Y':
//System.err.println("Y" + i + " " + d + " " + c + " " + di + " : " + dataset[di+c] + " " + axes[1].getIntPosition(dataset[di+c]) + "   " + (yb - axes[1].getIntPosition(dataset[di+c])) + " @ " + y + " " + yb);
            pnts[d++][i] = yb - axes[1].getIntPosition(dataset[di+c]);
            break;
          default:
          break;
          } 
