/*
 * @(#) $RCSfile: AbstractAxis.java,v $ $Revision: 1.1 $ $Date: 2004/01/28 20:32:55 $ $Name: TableView1_2 $
 *
 ...
 * @author       J Johnson
 * @version $Revision: 1.1 $ $Date: 2004/01/28 20:32:55 $  $Name: TableView1_2 $ 
 * @since        AV1.0
 */
public class AbstractAxis implements Axis, Cloneable {
  ...
  /** 
   * Create a AbstractAxis with default values: 
   * min = -10, max = 10, and size = 0.
   */
  public AbstractAxis() {
  }
  /** 
   * Create a AbstractAxis with default values of min = -10, max = 10 
  ...
  /** 
   * Create a AbstractAxis with the same min, max, and size values. 
   * as the given axis.
   ...
   * @param min the minimum value at the start of the axis.
   * @param max the maximum value at the end of the axis.
   */
  ...
  /**
   * Sets the range of the Axis.
   * @param range an array of length 2 holding the minimum and Maximum values of the axis.
   * @throws NullPointerException if the range is null.
   * @throws IllegalArgumentException if the range is not of length 2.
   */
  public void setRange(double[] range) throws NullPointerException, IllegalArgumentException {
    if (range == null) {
      throw new NullPointerException("setRange: range cannot be null");
    } else if (range.length != 2) {
      throw new IllegalArgumentException("setRange: range must be [min, max]");
    } else {
   ...
   * @return an array of length 2 holding the minimum and Maximum values of the axis.
   */
   ...
   *  @param tickIncrement The values at which to place the ticks.
   */
  public void setTicksByIncr(double tickIncrement) { 
    double tIncr =  tickIncrement;
    ...
    for (int size = getSize(); count > size; count /= 2) {
        tIncr *= 2;
    }
    ticks = new double[count];
    for (int i = 1; i < ticks.length; i+=2) {
      double val = smin + i/2 * tIncr;
      ticks[i-1] = getIntPosition(val);
      ticks[i] = val;
    }
  }
   ...
   *  Attempt to place the ticks at major numbers, for example:
  ...
  public void setTicks() {
    ...
    if (!Double.isNaN(tickIncrement) && tickIncrement != 0.) {
    ...
    for (int i = 0; i < count; i += 2) {
      ...
      double val = Math.round(sval * scale)/scale;
...
//System.err.println("getIntPosition " + value);
   ...
   * @param listener the AxisListener to add
   */
   ...
   * @param listener the AxisListener to remove
   */
