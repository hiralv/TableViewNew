/*
 * @(#) $RCSfile: AbstractGraphItem.java,v $ $Revision: 1.7 $ $Date: 2004/02/23 17:38:48 $ $Name: TableView1_2 $
 *
 ...
 * It provides common methods to set the data values for the item to be 
 * displayed.  The data values are interpreted by a DataModel that will 
 * attempt to convert any array into an array of double values that can 
 * be positioned by the axes of the graph.
 * @author       J Johnson
 * @version $Revision: 1.7 $ $Date: 2004/02/23 17:38:48 $  $Name: TableView1_2 $ 
 * @since        1.0
 * @see DataModel
 */
public abstract class AbstractGraphItem implements GraphItem {
   ...
   * @param indexedColor the color for each drawable of this graph item.
   */
   ...
   * @param rawData an array of x and y values 
   * @param format a String indicating the position of X and Y values 
   * in the array.
   * @see GraphDataModel
   * @see GraphDataModel#FORMAT_Y
   * @see GraphDataModel#FORMAT_XY
   */
  public void setData(Object rawData, String format) throws IllegalArgumentException {
    GraphDataModel dm = dataModel != null && dataModel instanceof GraphDataModel ? (GraphDataModel)dataModel : new GraphDataModel(); 
   ...
   * @param dataModel provides the x and y values for the graphed item. 
   * @see GraphDataModel
   */
  public void setData(DataModel dataModel) {
    if (this.dataModel != null &&  this.dataModel instanceof MutableDataModel) {
      ((MutableDataModel)this.dataModel).removeChangeListener(dataListener);
    }
   ...
   * @return a dataModel that provides the x and y values for the graphed item. 
   * @see GraphDataModel
   */
   ...
   */
  public abstract void draw(Component c, Graphics g, Rectangle r, 
  ...
  protected void fireChangeEvent() {
    ...
    for (int i = listeners.length-2; i>=0; i-=2) {
      if (listeners[i]==GraphItemListener.class) {
        ...
        ((GraphItemListener)listeners[i+1]).graphItemChanged(graphItemEvent);
      }
    }
  }
