/*
 * @(#) $RCSfile: Cells.java,v $ $Revision: 1.13 $ $Date: 2004/08/12 21:00:02 $ $Name: TableView1_3 $
 *
...
public class Cells {
  ...
  /**
   * Compares two objects as Numbers, if the Objects are Strings 
   * it will attempt to parse them as Numbers.
   */
  ...
  /** 
   * A convience method to return the column index for the 
   * given column name.  It tries first for a case sensitive 
   * match, then a case insensitive match.
   * @param tm The TableModel 
   * @param name The column heading to search for.
   * @return The index of the column (0 to columnCount-1), 
   *         or -1 if not found.
   */
   ...
   * @param indexArray An index array that is sorted to provide 
   *        an index from the orignal order.
   */
  ...
  /**
   * Get a sort index array for the list of values.
   * The list itself will not be sorted.
   * @param cells The values to sort.
   * @return An index array that provides that accesses the list 
   *         in sorted order.
   */
  ...
  /**
   * Get a sort index array for the list of values.  
   * Use the comparator given to compare values of the list.
   * The list itself will not be sorted.
   * @param cells The values to sort.
  ...
  /** 
   * Compare rows from a table based on the values of the columns 
   * in the given columnIndex array.  
   * @param tm The tableModel to be sorted.
   * @param lsm The row selection model for the tableModel to be sorted.
   * @param columnIndex The column indices on which to sort,
   *    The value for a column index is interpretted as follows:
   *       if the index == ROWNUM_ASCENDING sort by table row number
   *       if the index == ROWNUM_DESCENDING sort by reverse table row number
   *       if the index == SELECTED_ROWS_FIRST sort by row selection status
   *       if the index == SELECTED_ROWS_LAST sort by row selection status
   *       if positive sort ascending, 
   *       if negative sort descending and the column index = Math.abs(value) - 1.
   * @param rowIndex1 The first row index of the rows to compare.
   * @param rowIndex2 The other row index of the rows to compare.
   * @return negative if rowIndex1 should be sorted before rowIndex2, 
             positive if rowIndex1 is sorted after rowIndex2, else zero.
   ...
   * returning the order in an index array.
   * Compare rows from a table based on the values of the columns
   * in the given columnIndex array.
   * @param tm The tableModel to be sorted.
   * @param lsm The row selection model for the tableModel to be sorted.
  ...
  /**
   * Return the classes that are common to all members of
  ...
  /**
   * Return the most specific class that is common to all members of
   * the given collection.
  ...
  /**
   * Return the interfaces that are common to all members of
   * the given collection.
   * @param collection The Collection to examine.
