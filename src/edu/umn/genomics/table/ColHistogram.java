/*
 * @(#) $RCSfile: ColHistogram.java,v $ $Revision: 1.25 $ $Date: 2004/08/02 20:23:39 $ $Name: TableView1_3 $
 *
...
/**
 * Display a histogram of the values for each column of a table.
 * The display may be used to select rows of the table.
 * @author       J Johnson
 * @version $Revision: 1.25 $ $Date: 2004/08/02 20:23:39 $  $Name: TableView1_3 $ 
 * @since        1.0
 * @see  ColumnMap
 * @see  TableContext
 ...
 */
public class ColHistogram extends AbstractTableModelView 
    ...
    public void mouseReleased(MouseEvent e) {
      ...
      for(int c = 0; c < hist.length; c++) {
        if (hist[c] == h) {
          ...
          double from = cmap.getMin() + (sr[0]<sr[1]?sr[0]:sr[1]) * cr;
          double to   = cmap.getMin() + (sr[0]<sr[1]?sr[1]:sr[0]) * cr;
  ...
  ItemListener showPlot = new ItemListener() {
    public void itemStateChanged(ItemEvent e) {
      int c = Integer.parseInt(((JCheckBox)e.getSource()).getActionCommand());
   ...
   * @param tableModel contains the table data
   * @param lsm is the row selection model
   */
   ...
   * @param c data column for the histogram.
  ...
  public static Icon makeIcon(ColumnMap c, int w, int h) {
    ...
    if (c.getState() == CellMap.MAPPED) {
      int cnts[] = c.getCounts(c.isNumber()||c.isDate()?w:0,showSelection);
  ...
  private void setIcons() {
    ...
    for (int c = 0; c < ncols; c++) {
      ...
      JLabel jl = new JLabel(tm.getColumnName(c).substring(0,len < 5 ? len : 5));
      jl.setFont(lblFont);
  ...
  private void setAll() {
    ...
    if (hist != null && hist.length > 0) {
      dataNeeded = false;
      int ncols = hist.length < tm.getColumnCount() ? 
                                hist.length : 
                                tm.getColumnCount();
      for (int c = 0; c < ncols; c++) {
  ...
  private void setHistogramData(Histogram h, ColumnMap c) {
    if (h != null && c != null) {
      ...
      int cnts[] = c.getCounts(c.isNumber()||c.isDate()?w:0,doSelectCounts);
      ...
      if (vmax > 5) {
        ...
        for (int i = 0; i < ticks.length; i++) {
          ticks[i] = (double)i * scale;
        }
        vaxis[ci].getAxis().setTicks(ticks);
      }
      h.setVerticalAxis(vaxis[ci].getAxis());
      String pat = "#############";
      ((DecimalFormat)vaxis[ci].getAxisLabeler()).applyPattern(pat.substring(0,d+1));
  ...
  private void showHistogram(int c) {
    if (tm == null)
      return;
    if (c < 0 || c >= tm.getColumnCount() || hist == null || c >= hist.length)  
      return;
      ...
      haxis[c].setAxisLabeler(new ColumnMapLabeler(ctx.getColumnMap(tm,c),20));
      ...
      jp.add(hist[c]);
         jp.setBorder(BorderFactory.createTitledBorder(
            ...
            TitledBorder.BELOW_TOP, //ABOVE_TOP,
            font,
            Color.black
         ));
  ...
  /**
   * Display a histogram for each column of data in tthe table.
   * usage: java edu.umn.genomics.table.ColHistogram filename
