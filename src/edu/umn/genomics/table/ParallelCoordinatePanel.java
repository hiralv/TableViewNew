/*
 * @(#) $RCSfile: ParallelCoordinatePanel.java,v $ $Revision: 1.36 $ $Date: 2004/08/18 17:54:08 $ $Name: TableView1_3 $
 *
...
/**
 * A ParallelCoordinatePanel displays the values of a table 
 * as a parallel coordinate display.   Each column of a table is 
 * represented as a coordinate line in the panel.  The coordinate lines 
 * are drawn as parallel lines in the display.  
 * Each column in the table is mapped to normalized values along a
 * parallel coordinate.  Numeric values (derived from the Number class)
 * are mapped with the minimum value at the bottom of the coordinate line 
 * and the maximum value at the top of the coordinate line.
 * Other Object types are mapped so that distinct values are evenly
 * spaced along the parallel coordinate for the column.
 * Each row of the table is plotted as a polyline across the coordinate lines.
 * Rows of the table may be selected by taking a range of values 
 * along any column coordinate line.
 * @author       J Johnson
 * @version $Revision: 1.36 $ $Date: 2004/08/18 17:54:08 $  $Name: TableView1_3 $ 
 * @since        1.0
 * @see  ColumnMap
 * @see  TableContext
 ...
 */
public class ParallelCoordinatePanel extends AbstractTableModelView
                                     implements Cloneable, Serializable {

  /**
   * This class manages the 2D array of Y values for the polylines. 
   * All changes to the size of the array are done here.
   * The dataPoints array has an array of Y Values for row of the table.
   * The X points are common for each row and are in colPos array.
   * To draw a polyline for table row r :
  ...
  class PointArray {
    ...
    /**
     * Add rows to an existing array, inserting them if fromRow is less 
     * than then current array length, or else appending the new rows.
     * The new length will be at least toRow+1 in length.
     * Since array length can not be modified, we must create a new array 
     * and assign each of the row arrays to it.
     */
    private synchronized void addRows(int fromRow, int toRow) {
      ...
      // copy the first contiguous rows, before the insertion point, to dPnts
      if (fromRow > 0) {
        System.arraycopy(dataPoints,0,dPnts,0,
                         fromRow < dataPoints.length ? fromRow : dataPoints.length);
      }
      // create and add the new rows in dPnts, start from the lesser of fromRow or dataPoints.length
      for (int r = fromRow < dataPoints.length ? fromRow : dataPoints.length; r <= toRow; r++) {
        dPnts[r] = new int[ncols];
      }
      // copy the last contiguous rows, after the insertion point, to dPnts
      if (fromRow < dataPoints.length) {
        System.arraycopy(dataPoints,fromRow,dPnts,toRow+1,dataPoints.length - fromRow);
      }
      dataPoints = dPnts;
    }

    /**
     * Delete the range of rows, from fromRow to toRow inclusive, 
     * from the data array.
     */
    private synchronized void delRows(int fromRow, int toRow) {
      ...
      if (toRow+1 < dataPoints.length) {
        System.arraycopy(dataPoints,toRow+1,dPnts,fromRow,dataPoints.length - (toRow+1));
      }
      dataPoints = dPnts;
    }
     ...
     * than the current number of columns, else appending columns so that colIdx 
    ...
    private synchronized void addCol(int colIdx) {
      ...
      for (int r = 0; r < dataPoints.length; r++) {
        ...
        if (colIdx < row.length) {
          System.arraycopy(row,colIdx,dataPoints[r],colIdx+1,row.length - colIdx);
        }
      }
    }
  ...
  class ColLbl extends JPanel {
    public void paintComponent(Graphics g) {
      ...
      if (cPos != null) {
        ...
        Color mg = new Color((bg.getRed()+fg.getRed())/2,(bg.getGreen()+fg.getGreen())/2,(bg.getBlue()+fg.getBlue())/2);
        ...
        for (int c = 0; c < cPos.length; c++) {
          ...
          int lm = c == 0 ? 0 : cPos[c] - (cPos[c] - cPos[c-1])/2 + 2;
          int rm = (c == cPos.length-1 ? getWidth() : cPos[c] + (cPos[c+1] - cPos[c])/2) - 6;
  ...
  /**
   * A widget that show whether the values are in ascending or descending order.
   */
      ...
      if (cPos != null) {
        ...
        for (int c = 0; c < cPos.length; c++) {
          ...
          if (true && colMap != null) {
            ...
            String sval = valObj != null ? valObj.toString() : "NULL";
  ...
  /**
   * The Parallel Coordinate display that draws a polyline for each row of the table.
   */
...
//      System.err.println("DispPC dPnts " + dataPnts);
      ...
      if (dataPnts != null && dataPnts.length > 0 && dataPnts[0] != null) {
        ...
        if (cPos.length == 1) {
          ...
          if (!lsm.isSelectionEmpty()) {
            for (int r = lsm.getMinSelectionIndex(); 
                     r <= lsm.getMaxSelectionIndex(); r++) {
              if (rsm != null && !rsm.isSelectedIndex(r)) 
                continue; 
              if (lsm.isSelectedIndex(r)) {
                try {
          ...
          for (int r = 0; r < dataPnts.length; r++) {
            ...
            if (!lsm.isSelectedIndex(r)) {
              g.setColor(getColor(r));
              g.drawPolyline(cPos, row, row.length < clen ? row.length : clen);
            }
          }
          // second pass:  draw selected lines
          if (!lsm.isSelectionEmpty()) {
      ...
      if (selectPos != null && selectionColumn >= 0) {
        int x1 = selectionColumn == 0 ? 0 : cPos[selectionColumn] - 
                 (cPos[selectionColumn] - cPos[selectionColumn-1])/2;
        int x2 = selectionColumn == cPos.length - 1 ? getWidth() : 
                 cPos[selectionColumn] + 
                 (cPos[selectionColumn+1] - cPos[selectionColumn])/2;
        g.setColor(Color.black);
      ...
      if (valueFrom != null) {
        ...
        int y = pressLoc.y > fm.getAscent() ? pressLoc.y : fm.getAscent();
      ...
      if (valueAt != null) {
        ...
        int y = movedLoc.y > fm.getAscent() ? movedLoc.y : fm.getAscent();
        g.setColor(Color.white);
        g.fillRect(x, y - fm.getMaxAscent(), l, fm.getHeight());
        g.setColor(Color.black);
        g.drawString(valueAt, x, y);
      }
//    System.err.println("DispPC DONE");
    }
  }
  ...
  /** A list of columns that need to be mapped to the dPnts array.  Elements are Integer. */
  private SortedSet columnsToMap = Collections.synchronizedSortedSet(new TreeSet());
  ...
  /** The mapping of columns from the display position index to the table column index. */
  private int colIdx[] = null;
  /** The axis to use for mapping column values to the Y value in the display. (indexed by table column)*/
...
/*
 ...
 +----+----+-----+-+------------------------------------+
 C    N    N     N | colLbl     column Names            |
 C    N    N ttP +-+------------------------------------+
 C    N tP N     S | colTval    max value labels        |
 C    N    +-----+-+------------------------------------+
 C    N    S tbP N | colMov     drag widget             |
 C    +----+-----+-+------------------------------------+
 C    C            |                                    |
 C mP C            | dispPC     parallel coordinates    |
 C    C            |                                    |
 C    +----+-----+-+------------------------------------+
 C    S      	 N | colDir     invert widget           |
 C    S bP       +-+------------------------------------+
 C    S          S | colBval    min value labels        |
 +----+----+-----+-+------------------------------------+
*/
  ...
  ListSelectionListener lsmListener = new ListSelectionListener() {
    public void valueChanged(ListSelectionEvent e) {
      if (!e.getValueIsAdjusting()) {
        boolean enableBtn = !((ListSelectionModel)e.getSource()).isSelectionEmpty();
            ...
            if (e.getClickCount() > 1) {
              ...
              if (e.isShiftDown()) {
                ...
                if (clsm.isSelectedIndex(tmColIdx)) {
                  clsm.removeSelectionInterval(tmColIdx, tmColIdx);
                } else {
                  if (clsm.isSelectionEmpty()) {
                    setSelectedColumn(c); 
                  }
      ...
      if (cPos != null) {
        if (highlightColumn >= 0 && highlightColumn < cPos.length) {
          ...
          if (x < cPos[highlightColumn]) {
            ...
            for (int c = highlightColumn + 1;
                     c < cPos.length && x > cPos[c];c++) {
      ...
      int cPos[] = colPos;
        if (cPos != null) {
          ...
          setCursor(Cursor.getPredefinedCursor(
              onColumn ? Cursor.MOVE_CURSOR : Cursor.DEFAULT_CURSOR));
        }
        repaint();
    }
    ...
    private void mouseMove(MouseEvent e) {
      ...
      int cPos[] = colPos;
        if (cPos != null) {
          ...
          setCursor(Cursor.getPredefinedCursor(
              onColumn ? Cursor.HAND_CURSOR : Cursor.DEFAULT_CURSOR));
        }
        repaint();
    }
    public void mouseExited(MouseEvent e) {
    ...
    public void mousePressed(MouseEvent e) {
      ...
      if (selectionColumn >= 0) {
          if (ctx != null) {
            ...
            valueFrom = getValueString(dval,ctx.getColumnMap(tm,cIdx[c])); 
            pressLoc.y = p.y;
            pressLoc.x = cPos[c];
          }
          selectRange = true;
...
try {
      ...
      if (selectRange) {
        ...
        if (selectionColumn >= 0 && selectionColumn < cPos.length) {
          ColumnMap colMap = ctx.getColumnMap(tm,cIdx[selectionColumn]);
          String nullSelected = null;
          double from = getColumnValue(selectionColumn,selectPos[0]);
          double to   = getColumnValue(selectionColumn,selectPos[1]);
          if (Double.isNaN(from) || Double.isNaN(to)) {
            nullSelected = colMap.getName()+" = NULL ";  //?check SQL spec
            // If either is non null, then we may need to swap to and from values
            //  in order for the ColumnMap to select the right range
            // Find out if drag was in order of increasing or decreasing order
            //  Is NaN range next to PosInf or NegInf
            int dy = selectPos[1] - selectPos[0];
            double mid = getColumnValue(selectionColumn,(selectPos[0]+selectPos[1])/2);
            if (!Double.isNaN(from)) {
              if (Double.isInfinite(from)) {
                if (Double.isInfinite(mid) || Double.isNaN(mid)) {
                  from = Double.NaN;
                }
              } else if (mid < from) {
                to = from;
    ...
    public void mouseMoved(MouseEvent e) {
      ...
      if (cPos != null) {
        ...
        for (int c = 0; c < cPos.length; c++) {
          if (cPos[c] >= p.x - hitWidth && cPos[c] <= p.x + hitWidth) {
            ColumnMap cmap = ctx.getColumnMap(tm,cIdx[c]);
            setCursor(Cursor.getPredefinedCursor(cmap.getState() == CellMap.MAPPED 
                       ? Cursor.CROSSHAIR_CURSOR : Cursor.WAIT_CURSOR)); 
            movedLoc = p;
              ...
              if (dataPnts != null) {
                ...
                for (int r = 0; r < dataPnts.length; r++) {
                  if (dataPnts[r][c] == p.y) {
                    ...
                    int d = p.y > dataPnts[r][c] ? p.y - dataPnts[r][c] 
                                                 : dataPnts[r][c] - p.y;
                    if (d < diff) {
                      closestRow = r;
                      diff = d;
                ...
                if (cmap.isNumber() || cmap.isDate()) {
                  ...
                  Object obj = tm.getValueAt(closestRow,cIdx[c]);
                  if (obj != null) {
                    valueAt = tm.getValueAt(closestRow,cIdx[c]).toString();  
        ...
        new ListSelectionListener() {
          public void valueChanged(ListSelectionEvent e) {
            if (!e.getValueIsAdjusting()) {
              ...
              showBtn.setEnabled(indices.length < colList.getModel().getSize());
            }
          }
        });
    // toolbar
  ...
  /**
   * Constructs a Parallel Coordinate display.  Nothing will be displayed 
   ...
   * performed by the ParallelCoordinatePanel when it receives the event.
   ...
   * @param r the row being inquired about
  ...
  private int[] getSelectIndices(ListSelectionModel aLsm) {
    ...
    if (!aLsm.isSelectionEmpty()) {
      int d = aLsm.getMaxSelectionIndex() - aLsm.getMinSelectionIndex() + 1;
      int n = 0;
      int[] tmp = new int[d];
      for (int i = aLsm.getMinSelectionIndex(); i <= aLsm.getMaxSelectionIndex(); i++) {
  ...
  private void setSelectedColumn(int colPos) {
    selectedColumn = colIdx != null && colPos >= 0 && colPos < colIdx.length ? colIdx[colPos] : 0;
  }

  /**
   * Returns the Column display index of the selectedColumn
   * return the Column display index of the selectedColumn.
   */
  ...
  /**
   * Returns the Column display index of the given table column.
   * return the Column display index of the given table column.
   */
   ...
   * @param columns the indices of the TableModel columns to display.
   */
   ...
   * @param columns the indices of the columns that should be plotted in the same range.
   */
  ...
  /**
   * Sets the table rows that will be displayed.
   * If selectedRows is null, all rows are displayed.
   * @param selectedRows if not null, only row indices in the array will be displayed.
   */
  ...
  /**
   * Selects table rows that will be displayed.
   * If the selection model is null, all rows are displayed.
   * @param rowDisplaySelection if not null, only row indices that are selected will be displayed.
   */
  ...
  private void showColumns(int[] columns) {
    int ncols = columns.length <= tm.getColumnCount() ? columns.length : tm.getColumnCount();
    int cPos[] = new int[ncols];
    int cIdx[] = new int[ncols];
    for (int c = 0; c < ncols; c++) {
      int i = columns[c];
  ...
  private void positionColumns() {
    ...
    if (cPos != null) {
      ...
      double colw = (double)mP.getWidth()/ncols; // space between column axes
      for (int c = 0; c < ncols; c++) {
   ...
   * @param c The table model index of the column.
   ...
   * @param cmap The table column map.
   * @param axis The axis for table to display mapping.
   * @param invert Whether to invert the column in the display.
   */
   ...
   * @param c The display position index for the column.
   */
   ...
   * @param c An array of display position indices for columns.
   */
  private synchronized void mapData(int[] cPos) {
    if (cPos != null) {
      for (int c = 0; c < cPos.length; c++) {
  ...
  /** 
   * Starts the thread to map the data values for each row in the table 
   * to polyline values for each column in the columnsToMap list.
   */
  ...
  /** 
   * Map the data values for each row in the table to polyline values for 
   * each column in the columnsToMap list.
   * The mapping depends on the size of the display so this method should 
   * be called whenever the view size changes.
   */
   ...
   * @param col The display position index for the column.
   * @param loc The y coordinate of the display.
   ...
   * @param dataPnts The array of polyline y points.
   * @param c The display position index for the column.
   */
        ...
        if (colMap.getState() != CellMap.MAPPED) {
          ...
          for (int r = 0; r < nrows && r < dataPnts.length; r++) {
            if (Thread.currentThread().interrupted())
              return;
            dataPnts[r][c] = axis.getIntPosition(colMap.getMapValue(r));
          }
        }
      }
    } catch (Throwable ex) {
   ...
   * @param dataPnts The array of polyline y points.
   * @param fromRow The first row index to map
   * @param toRow The last row index to map
   */
  public void mapRowValues(int[][] dataPnts, int fromRow, int toRow) {
    ...
    for (int r = fR; r < tR && r < dataPnts.length; r++) {
      ...
      for (int c = 0; c < row.length && c < colIdx.length; c++) {
        ...
        Axis axis = getAxis(tmColIdx);
        ColumnMap colMap = ctx.getColumnMap(tm,tmColIdx);
        if (axis != null && colMap != null && colMap.getState() == CellMap.MAPPED) {
  ...
  public Object clone() throws CloneNotSupportedException {
    ...
    try {
      ...
      pcp.colPos = (int[])colPos.clone();
      pcp.partitionMap = partitionMap != null ? (PartitionIndexMap)partitionMap.clone() : null;
      pcp.axisList.clear(); 
      for(int i = 0; i < axisList.size(); i++) {
        pcp.axisList.add((Axis)((Axis)axisList.get(i)).clone());
      }
  ...
  /**
   * Return a JTable that displays all the distinct values of the given table column 
   * in the first column, and a ParallelCoordinates graph of the table rows that 
   * match the respectitive value in the second column.
   * @param tc The index of the TableModel column on which this table will be built.
   * @return A JTable displaying graphs for each distinct value of the column in the table.
   */
  public JTable getTableDisplay(int tc) {
    ...
    ParallelCoordDisp pcd = new ParallelCoordDisp(getTableContext(), tm, colIdx, getAxes());
  ...
  /**
   * This can display a ParallelCoordinate graph as either an Icon or as a TableCellRenderer.
   */
  class ParallelCoordDisp extends JComponent implements TableCellRenderer, Icon {
    ...
    ParallelCoordDisp(TableContext cxt, TableModel tm, int columns[], Axis axis[]) {
      this.cxt = cxt;
      this.tm = tm;
    ...
    ParallelCoordDisp(TableContext cxt, TableModel tm, int columns[], Axis axis[], ListSelectionModel rsm) {
    ...
    public void paintComponent(Graphics g) {
      ...
      for (int c = 0; c < ncol; c++) {
        ...
        axis[tc].setSize(iH);
        g.drawLine(xPos[c],inset.top,xPos[c],getHeight()-inset.bottom);
      ...
      // set and draw yPos
      if (rsm != null) {
        for (int r = rsm.getMinSelectionIndex(); r <= rsm.getMaxSelectionIndex(); r++) {
  ...
  class AxisRenderer extends AxisDisplay implements TableCellRenderer {
    /**
     * Create an AxisRenderer with position defaulting to AxisComponent.BOTTOM.
     */
    public AxisRenderer() {
      setAxis(new LinearAxis());
    }
    /**
     * Create an AxisRenderer with the given axis and default position
     * of AxisComponent.BOTTOM.
     ...
     * of AxisComponent.BOTTOM.
     * @param axis the axis to display
     * @param position position of the axis display releative to the graph.
     */
  ...
  /**
   * Display a parallel coordinate view of table of data.
   * usage: java edu.umn.genomics.table.ParallelCoordinatePanel filename
   * The file should have a format that the FileTableModel class can read.
   * If there are no arguments, random data will be generated.
   * @param args the filename of the table data
   * @see FileTableModel
   */
  public static void main(String[] args) {
    ...
    if (args.length > 0) {
      ...
      for (int i = 0; i < 30; i++) {
        ...
        v.addElement(pfams.elementAt((int)(Math.random() * pfams.size())));
