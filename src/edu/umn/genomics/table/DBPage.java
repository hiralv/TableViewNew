/*
 * @(#) $RCSfile: DBPage.java,v $ $Revision: 1.2 $ $Date: 2004/02/11 15:53:00 $ $Name: TableView1_3 $
 *
...
public class DBPage {
   ...
   * @param pageSize The number of rows the query should attempt to return.
   * @param page The page of rows to return, ( OFFSET = pageSize * (page - 1) )
   * @return A modified query that will provide the row LIMIT and OFFSET, 
   *         else null if this was unable to build such a query for this database.
   */
  public static String getPageQuery(DatabaseMetaData dbmd, String query, int pageSize, int page) {
    ...
    if (dbname != null && dbname.equalsIgnoreCase("Oracle")) {
      ...
      if (dbMajorVersion > 4)  {
        ...
        if (dbMinorVersion < 1 && dbProductVersion.compareTo("4.0.6") < 0) {
    ...
    if (supportsLimit && supportsOffset) {
      ...
      if (lsi >= 0) {
        ...
        // Now find the first occurance of any character that is not 0-9 or comma
        ...
        if (osi >= 0) {  
          ...
          // Now find the first occurance of any character that is not 0-9 or whitespace
          ...
          sql = query.substring(0,lsi) 
              + "\n" + limitKw + " " + (offset > 0 ? (offset + "," + nrow) : ("" + nrow))
              + (lei < query.length() ? query.substring(lei) : "");
        }
      } else {
        sql = query + "\n" + limitKw + " " + nrow + " " + offsetKw + " " + offset;
      }
...
/*
      select  
      from 
        select my_tmp*, ROWNUM SBQ_ROW_NUM 
      where RN > 
*/
    } else {
      sql = null;
