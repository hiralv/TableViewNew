/*
 * @(#) $RCSfile: Histogram3dView.java,v $ $Revision: 1.1 $ $Date: 2004/08/02 20:23:41 $ $Name: TableView1_3 $
 *
 ...
 * of the table.
 * @author       J Johnson
 * @version $Revision: 1.1 $ $Date: 2004/08/02 20:23:41 $  $Name: TableView1_3 $ 
 * @since        1.0
 * @see  ColumnMap
 * @see  TableContext
...
public class Histogram3dView extends AbstractTableModelView
  ...
  private void init() {
    ...
    canvas = ctx != null ? new Histogram3dDisplay(hgm,ctx.getSetOperator(tm)) 
                       : new Histogram3dDisplay(hgm);

    // top.add(new JLabel("X: "));
    xChoice.setBorder(BorderFactory.createTitledBorder(
    ...
    top.add(xChoice);
    yChoice.setBorder(BorderFactory.createTitledBorder(
                       BorderFactory.createEtchedBorder(),
                       "Column Y",
                       TitledBorder.DEFAULT_JUSTIFICATION,
                       TitledBorder.BELOW_TOP));
    ...
    spinner = new JSpinner(new SpinnerNumberModel(canvas.getBarWidth(), .1, 1., .1));
    ...
    spinPanel.setBorder(BorderFactory.createTitledBorder(
                       BorderFactory.createEtchedBorder(),
                       "Bar Size",
                       TitledBorder.DEFAULT_JUSTIFICATION,
                       TitledBorder.BELOW_TOP));
    ...
    selectOnly.addItemListener(new ItemListener() {
        public void itemStateChanged(ItemEvent e) {
          canvas.setSelectBarsOnly(e.getStateChange()==ItemEvent.SELECTED);
        }
    });
    top.add(selectOnly);
    ...
    add(plotPanel);
    tilt = new JSlider(JSlider.VERTICAL, 0, 500,  500 - (int)(canvas.getTilt() * 1000));
    ...
    rotation = new JSlider(JSlider.HORIZONTAL, -1000, 1000, (int)(canvas.getRotation() * 1000));
   ...
   * @param x the index of the column for the x axis.
   * @param y the index of the column for the y axis.
   */
