/*
 * @(#) $RCSfile: VirtualTableModelView.java,v $ $Revision: 1.13 $ $Date: 2004/08/18 17:37:22 $ $Name: TableView1_3 $
 *
 ...
 *
 * @author       J Johnson
 * @version $Revision: 1.13 $ $Date: 2004/08/18 17:37:22 $  $Name: TableView1_3 $ 
 * @since        1.0
 * @see  JSFormula
...
public class VirtualTableModelView extends JPanel {
  ...
  Object[][] columnTypeInfo = { // Name, Class, VirtualCell
    {"Table Column",edu.umn.genomics.table.ColRef.class,edu.umn.genomics.table.ColRef.class},
    {"Text",java.lang.String.class,edu.umn.genomics.table.VectorColumn.class},
    {"Integer",java.lang.Integer.class,edu.umn.genomics.table.VectorColumn.class},
    {"Number",java.lang.Double.class,edu.umn.genomics.table.VectorColumn.class},
    {"Date",java.util.Date.class,edu.umn.genomics.table.VectorColumn.class},
    {"Boolean",java.lang.Boolean.class,edu.umn.genomics.table.VectorColumn.class},
    // {"Image",java.awt.Image.class,edu.umn.genomics.table.VectorColumn.class},
    // {"Color",java.awt.Color.class,edu.umn.genomics.table.VectorColumn.class},
    {"BeanShell",edu.umn.genomics.table.BshFormula.class,edu.umn.genomics.table.BshFormula.class},
    {"JavaScript",edu.umn.genomics.table.JSFormula.class,edu.umn.genomics.table.JSFormula.class},
  };
  ...
  class ColumnTableModel extends AbstractTableModel {
    ...
    public String getColumnName(int column) {
      return column >= 0 && column < columnNames.length 
               ? columnNames[column]
    ...
    public void setValueAt(Object aValue, int row, int column) {
      switch (column) {
      case 0:
       vtm.setColumnName(aValue != null ? aValue.toString() : null,column); 
        break;
      case 1:
        break;
      case 2:
      ...
      case 3:
        try {
          VirtualCell vcell = (VirtualCell)vtm.getColumnList().getElementAt(row);
          if (vcell instanceof TableModelFormula) {
            ((TableModelFormula)vcell).setFormula(aValue != null ? aValue.toString() : "");
            if (vtm != null && vtm instanceof AbstractTableModel) {
              ((AbstractTableModel)vtm).fireTableChanged(
                new TableModelEvent(vtm, 0, vtm.getRowCount()-1, row, TableModelEvent.UPDATE));
            }
          }
        } catch (Exception ex) {
        }
  ...
  JSplitPane split = new JSplitPane(JSplitPane.VERTICAL_SPLIT,colPanel,viewPanel);
  ...
  public VirtualTableModelView(VirtualTableModel tableModel) {
    ...
    colLSM.addListSelectionListener(
          ...
          new ActionListener() {
            public void actionPerformed(ActionEvent e) {
              if (vtm.getTableModel() != null && 
                ...
                DefaultTableModel dtm = (DefaultTableModel)vtm.getTableModel();
    ...
    rowChangesAllowed = vtm.getTableModel() != null && vtm.getTableModel() instanceof DefaultTableModel;
    ...
    cjsp.setPreferredSize(colTable.getPreferredSize());
    colPanel.add(cjsp);
...
//   VectorColumn
  ...
  public void newColumn(int columnIndex) {
    ...
    tmCols.addListSelectionListener( 
      new ListSelectionListener() {
        public void valueChanged(ListSelectionEvent e) {
    ...
    final JScrollPane hjsp = new JScrollPane(editorPane,
                                             JScrollPane.VERTICAL_SCROLLBAR_ALWAYS,
                                             JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);

    sjsp.setBorder(BorderFactory.createTitledBorder("Enter Script"));
    hjsp.setBorder(BorderFactory.createTitledBorder("Script Examples"));
    final JSplitPane ssplit = new JSplitPane(JSplitPane.VERTICAL_SPLIT,sjsp,hjsp);
    ssplit.setOneTouchExpandable(true);
    ssplit.setDividerLocation(.9);
    ssplit.setResizeWeight(.9);
    ssplit.setPreferredSize(new Dimension(600,240));
    ...
    tjsp.setVisible(edu.umn.genomics.table.ColRef.class.isAssignableFrom(selClass));
    ssplit.setVisible(edu.umn.genomics.table.TableModelFormula.class.isAssignableFrom(selClass));
    columnType.addItemListener(
      new ItemListener() {
          ...
          public void propertyChange(PropertyChangeEvent e) {
            ...
            if (dialog.isVisible() 
              ...
              if (optVal == options[0]) {
                ...
                if (colName.length() > 0) {
                  if (edu.umn.genomics.table.ColRef.class.isAssignableFrom(selClass)) {
                    int idx = tmCols.getSelectedIndex();
                    if (idx >= 0) {
                      newColumn(new ColRef(colName,vtm.getTableModel(),idx), colIdx[0]);
                    }
                  } else if (edu.umn.genomics.table.TableModelFormula.class.isAssignableFrom(selClass) ) {
                    try {
  ...
  /**
   * Display a VirtualTableModelView
   * usage: java edu.umn.genomics.table.VirtualTableModelView 
   * @see FileTableModel
   */
