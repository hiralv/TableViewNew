/*
 * @(#) $RCSfile: JDBCTableModel.java,v $ $Revision: 1.14 $ $Date: 2004/05/13 14:42:44 $ $Name: TableView1_3 $
 *
 ...
 * @author       J Johnson
 * @version $Revision: 1.14 $ $Date: 2004/05/13 14:42:44 $  $Name: TableView1_3 $
 * @since        1.0
 */
public class JDBCTableModel extends AbstractTableModel implements TableColumnMap {
/* 
*/
 ...
 // Listeners 
  ...
  /**
   * Create a JDBCTableModel, the database account must be supplied by 
   * the setUser method.
   */
   ...
   */
  public JDBCTableModel(DBConnectParams dbparams) throws SQLException, ClassNotFoundException {
    setUser(dbparams);
  }
 ...
 // interface QueryTableModel
   ...
   * @param dbparams the database user account information.
   */
  public void setUser(DBConnectParams dbparams) throws ClassNotFoundException,SQLException {
    if (debug > 0) System.err.println("setUser " + "\t" + dbparams);
    if (dbparams.getDriverName() != null && dbparams.getDriverName().length() > 0) {
      Class.forName(dbparams.getDriverName());
      Driver driver = DriverManager.getDriver(dbparams.getURL());
    }
    Connection conn = DriverManager.getConnection(dbparams.getURL(), dbparams.getUser(), 
  ...
  synchronized public int getCountForQuery(String query) {
    if (query != null) {
      if (debug > 1) System.err.println("getCountForQuery:\t" + query ); 
      int count = -1;
      try {
   ...
   * @param pageSize The number of rows to retrieve. (The SQL99 LIMIT parameter)
   * @param page The page of rows to retrieve (starting from page 1). 
   *             (The SQL99 OFFSET parameter = pageSize * (page-1) ).
   */
  ...
  synchronized private void setDataQuery(String query) {
    if (debug > 0) System.err.println("setQuery " + "\t" + query);
    setReadCount(0);
    ...
    if (dbparams != null) {
      ...
      dataThread = new Thread () {
        public void run() {
          try {
            if (debug > 0) System.err.println("dataThread " + "\t" + getQuery());
  ...
  private void addStatement(Statement stmt) {
    ...
    try {
      if(debug>3) System.err.println("stmtv add: " + stmt.getConnection() + " " + stmtv);
    } catch (Exception ex) {
      if(debug>3) System.err.println("stmtv add: " + ex);
    }
  }
  private void removeStatement(Statement stmt) {
    stmtv.removeElement(stmt);
    if(debug>3) System.err.println("stmtv rmv: " + stmtv);
  }
  ...
  private void addResultSet(ResultSet rslt) {
    ...
    try {
      if(debug>3) System.err.println("rsltv add: " + rslt.getStatement().getConnection() + " " + rsltv);
    } catch (Exception ex) {
      if(debug>3) System.err.println("rsltv add: " + ex);
    }
  }
  ...
  private void removeResultSet(ResultSet rslt) {
    ...
    if(debug>3) System.err.println("rsltv rmv: " + rsltv);
  }
  ...
  public ResultSet execQuery(String sql,boolean isCount) throws SQLException {
    if (debug > 1) System.err.println("execQuery " + "\t" + sql);
    DBConnectParams dbusr = getUser();
    Connection conn = DriverManager.getConnection(dbusr.getURL(), dbusr.getUser(), 
                                                  dbusr.getPassword());
    ...
    if (debug > 4) System.err.println("execQuery " + "\t" + stmt);
    ResultSet rs = stmt.executeQuery(sql);
    if (debug > 4) System.err.println("execQuery " + "\t" + rs);
    if (!isCount) {
      addResultSet(rs);
      removeStatement(stmt);
    }
  ...
  /** 
   * Return a query the will get the min and max values 
   * for the given column for the query.
   * @param colName The name of the column from the query.
   * @return the SQL query.
   */
  ...
  /** 
   * Return a query the will get the variance 
   * for the given column for the query.
   * @param colName The name of the column from the query.
   * @return the SQL query.
   */
  ...
  /** 
   * Return a query the will get the average
   * for the given column for the query.
   * @param colName The name of the column from the query.
   * @return the SQL query.
   */
  ...
  /** 
   * Return a query the will get the count of distinct values
   * for the given column for the query.
   * @param colName The name of the column from the query.
   * @return the SQL query.
   */
  ...
  /** 
   * Return a query the will get the count of null values
   * for the given column for the query.
   * @param colName The name of the column from the query.
   * @return the SQL query.
   */
  ...
  private String getStatQuery(String selectClause, String whereClause) {
    ...
    if (dbname != null && dbname.equalsIgnoreCase("Oracle")) {
      ...
      sql = "select " + selectClause + " from (" + getQuery() + ") derivedtable";
      if (whereClause != null) {
        sql += " where " + whereClause;
      }
    } else {
      ...
      if (debug > 2) System.err.println("getStatQuery " + sql);
    }
    return sql;
  }
  ...
  private Thread collectStatsThread(final DBColumnMap cMap) {
    Thread statsThread = new Thread () {
      ...
      public void run() {
        try {
          if (debug > 3) System.err.println("statsThread " + "\t" + cmap.getName());
          collectStats(cmap);
        } catch (Exception ex) {
  ...
  private void collectStats(DBColumnMap cmap) {
    ...
    try {
      ...
      if (!supportsSelectInFrom()) {
        ...
        if (fields.equals("*")) {
          ...
          String field = ((String)fv.get(cmap.getColumnIndex())).trim();
          if (field.indexOf(' ') > 0) {
    ...
    try {
      if (debug > 1) System.err.println("collectStats:\t" + q );
      ...
      try {
        if (rs != null && rs.next()) {
          ...
          if (cmap.isNumber()) {
            ...
            cmap.min = (double)((java.util.Date)cmap.minObj).getTime();
            cmap.max = (double)((java.util.Date)cmap.maxObj).getTime();
          } else {
            cmap.min = 0.;
          }
        }
    ...
    if (cmap.isNumber()) {
      ...
      try {
        if (debug > 2) System.err.println("collectStats:\t" + q );
        ResultSet rs = execQuery(q, false);
        try {
          if (rs != null && rs.next()) {
    ...
    if (true) {
      ...
      try {
        ...
        try {
          if (rs != null && rs.next()) {
            cmap.setDistinctCount(((Number)rs.getObject(1)).intValue());
  ...
  /**
   * Read the ResultSet data for the query, creating columnMaps to
   * hold the values for each column.
   */
  private void readData(ResultSet rs) throws SQLException {
    // determine columns classes
    ResultSetMetaData rsmd = rs.getMetaData();
    int ncol = rsmd.getColumnCount();
    if (debug > 2) System.err.println("Number of Columns: " + ncol);
    // create ColumnMaps
    DBColumnMap cMap[] = new DBColumnMap[ncol];
    for (int c = 0; c < ncol; c++) {
      String name = rsmd.getColumnLabel(c+1);
      if (debug > 2) System.err.println("  Column " + c + "\t" + name);
      ...
      for (int r = 0;(rowLimit < 0 || r < rowLimit) && rs.next();r++ ) {
        ...
        if (rcnt % updateIncr == 0) { 
          if (r + 1 == rowCount) {
            ...
            fireStatusEvent("", rcnt, rowLimit < 0 ? -1 : rowLimit, queryCount, null);
          } else {
            fireTableRowsInserted(lastCnt, r);
            fireStatusEvent("", rcnt, rowLimit < 0 ? rowCount : rowLimit, queryCount, null);
          }
          lastCnt = rcnt;
        }
      }
    ...
    for (int c = 0; c < ncol; c++) {
      ...
      if (debug > 3) System.err.println(c + " " + cMap[c].getName() + " " + cMap[c].getState() + " " + cMap[c].getMin() + " to " +  cMap[c].getMax());
    }
  }
 ...
 // interface TableModel
   ...
   */
  public String getColumnName(int columnIndex) throws ArrayIndexOutOfBoundsException {
    DBColumnMap cMap[] = colMap;
    if (cMap == null || columnIndex < 0 || columnIndex >= cMap.length) {
      throw new ArrayIndexOutOfBoundsException(columnIndex 
        + " is not a valid columnIndex in a table with " 
        + (cMap == null || cMap.length < 1 ? "no" : "" + cMap.length) + " columns.");
    }
   ...
   */
  public Class getColumnClass(int columnIndex)  throws ArrayIndexOutOfBoundsException {
    DBColumnMap cMap[] = colMap;
    if (cMap == null || columnIndex < 0 || columnIndex >= cMap.length) {
      throw new ArrayIndexOutOfBoundsException(columnIndex 
        + " is not a valid columnIndex in a table with " 
  ...
  public Object getValueAt(int rowIndex, int columnIndex) throws ArrayIndexOutOfBoundsException {
    ...
    if (rowIndex < 0 || rowIndex > rowCount) {
      throw new ArrayIndexOutOfBoundsException(rowIndex 
        + " is not a valid rowIndex in a table with " + rowCount + " rows.");
    }
    if (cMap == null || columnIndex < 0 || columnIndex >= cMap.length) {
      throw new ArrayIndexOutOfBoundsException(columnIndex 
        + " is not a valid columnIndex in a table with " 
...
//  public void setValueAt(Object aValue, int rowIndex, int columnIndex);
 ...
 // Status Listener
   ...
   * @param listener the StatusListener to add
   */
   ...
   *
   */
  protected void fireStatusEvent(String message, int rowsRead, int rowCount, int totalCount, Exception ex) {
    int offset = getRowOffset();
    String msg = (message != null ? message : "") 
               + (rowsRead < 0 ? "" : (" Read rows " + (offset + 1) + " to " + (offset + rowsRead)))
