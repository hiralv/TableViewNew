/*
 * @(#) $RCSfile: FileTableModel.java,v $ $Revision: 1.12 $ $Date: 2004/05/04 20:00:33 $ $Name: TableView1_3 $
 *
...
/** 
 * FileTableModel presents a TableModel interface to a data file.  
 * FileTableModel attempts to open the data source as a RandomAccessFile.  
 * If the data source is a URL, it will attempt to copy the data to 
 * a temporary file that it can access as a RandomAccessFile. 
 * FileTableModel wiil attempt to determine the field separator, 
 * the presence of header lines, 
 * and the data Class of each column (Character String, Number, or Date) 
 * by examining the first several lines of data. 
 * @author       J Johnson
 * @version $Revision: 1.12 $ $Date: 2004/05/04 20:00:33 $  $Name: TableView1_3 $ 
 * @since        1.0
 * @see  javax.swing.table.TableModel
 */
public class FileTableModel extends AbstractTableModel implements TableColumnMap, ListModel, Runnable {
  /*
     header pattern
     include pattern
     exclude pattern
   */
  /*
  // Eventually we want to present choices about converting this file to a TableModel
  class TableStats {
    public int getColumnCount();
    public Class[] getColumnClass();
    public int getHeaderRowCount();
    public void setHeaderRowCount(int headerRows);
    public Vector getPossibleFieldSeparators(String[] lines); 
    public String getFieldSeparator();
    public void setFieldSeparator(String fs);
    public char getTextFieldQuote();
    public void setTextFieldQuote(char quoteChar);
    // public boolean getTextFieldsQuoted();
    // public boolean setTextFieldsQuoted(boolean isQuoted);
  }
  */
  class ColStats {
    public String name = "";
    ...
    public String toString() {
      ...
      sb.append(brk);
      sb.append("numbers = " + numCnt + 
                 (numCnt > 0 ? (" [" + minNum + " - " + maxNum + "]"):""));
      sb.append(brk);
      sb.append("dates = " + dateCnt + 
 ...
 // Column info
  ...
  class Indexer extends Thread {
    ...
    public void run() {
      ...
      setIndexedCount();
      //System.err.println((this.rdr != null ? "copied " : " indexed ") + cnt + " lines");
    }
  }
...
//System.err.println(rowIndex + "\t" + rv);
      ...
      for (int c = 0; c < cMap.length; c++) {
        // System.err.println(c + " " + cMap[c].getName() + " " + cMap[c].getState() + " " + cMap[c].getMin() + " to " +  cMap[c].getMax() );
  ...
  class FileListModel extends AbstractListModel {
    ...
    public Object getElementAt(int index) {
      try {
        return index < getSize() ? readLine(index) : null;
  ...
  public Object getElementAt(int index) {
    return flm.getElementAt(index);
  }
  public void addListDataListener(ListDataListener l) {
    flm.addListDataListener(l);
  }
  public void removeListDataListener(ListDataListener l) {
    flm.removeListDataListener(l);
  }

  public JPanel getFileLinesView() {
    JPanel panel = new JPanel(new BorderLayout());
    JList fileLines = new JList(flm);
    JList lineNums = new JList(rowNumLM);
    JScrollPane jsp = new JScrollPane(fileLines);
    jsp.setRowHeaderView(lineNums);
    panel.add(jsp);
    return panel;
  }
  ...
  private synchronized void setIndexedCount() {
    ...
    //System.err.println(" size was " + cntWas + " size is " + vt.getSize());
  }
  ...
  /** 
   * FileTableModel presents a text file as a TableModel.
   * @param source The path or URL to the data source.
   */
  ...
  public void read(String source) throws IOException {
    ...
    dataRowOffset = guessHeaderLineCount(csa);
    if (dataRowOffset > 0) {
    ...
    for (int c = 0; c < csa.length; c++) {
      if (csa[c].firstNumDate < lcnt && csa[c].lastString <= csa[c].firstNumDate) {
        if (csa[c].numCnt > 0) {
      ...
      colMap[c] = new CacheColumnMap(this, null, getColumnName(c), c, columnClass[c]);     
    }
    setIndexedCount();
    try {
  ...
  public int guessHeaderLineCount(ColStats[] csa) {
    // determine header rows
    int lcnt = 0;
    int hi[] = new int[csa.length];
    for (int c = 0; c < csa.length; c++) {
      lcnt = Math.max(lcnt,csa[c].valCnt + csa[c].nullCnt);
    }
    for (int c = 0; c < csa.length; c++) {
      ...
      if (csa[c].firstNumDate < lcnt && csa[c].lastString < csa[c].firstNumDate) {
        hi[c] = csa[c].firstNonString;
      }
    }
    // The offset ocurring the most times is the most likely
    Arrays.sort(hi);
    int mCnt = 0;
    int mSize = hi[hi.length-1];
    for (int i = hi.length-1, cCnt = 0, cSize = mSize; i >= 0; i--) {
      if (hi[i] == cSize) {
        cCnt++;
      } else {
        if (cCnt > mCnt) {
  ...
  private int getLineOffsets(RandomAccessFile file) {
    ...
    try {
      for (String line = file.readLine(); line != null; 
           offset = file.getFilePointer(), line = file.readLine(), linesRead++) {
        // Ignore blank lines
        ...
        setLineOffset(linesRead,offset);
   ...
   * @param rdr The source data
   * @param file The local RandomAccessFile file to cache data.
   */
  private int copy(BufferedReader rdr, RandomAccessFile file) {
    ...
    try {
      for (String line = rdr.readLine(); line != null; 
           offset = writeLine(file, line, nl, offset), line = rdr.readLine(), linesRead++) {
        // Ignore blank lines
        while (line != null && line.length() == 0) {
          offset = file.getFilePointer();
          line = file.readLine();
  ...
  private synchronized long writeLine(RandomAccessFile file, String line, String nl, long offset) throws IOException {
    file.seek(offset);
  ...
  private RandomAccessFile getRandomAccessFile(String source) throws IOException {
    RandomAccessFile file = null;
    try {
  ...
  private long getRowOffset(int rowIndex) {
    return lineOffset[rowIndex + dataRowOffset];
  }
    
  private long getLineOffset(int lineIndex) {
    return lineOffset[lineIndex];
  }

  private void setLineOffset(int lineIndex, long offset) {
//System.err.println("setLineOffset " + lineIndex + "\t" + offset);
    final int incr = 1000;
    if (lineOffset == null) {
      lineOffset = new long[incr + lineIndex % incr];
    } else if (lineIndex >= lineOffset.length) {
      long tmp[] = lineOffset;
      lineOffset = new long[1000 + lineIndex / 1000 * 1000];
      System.arraycopy(tmp, 0 , lineOffset, 0, tmp.length);
    } 
    lineOffset[lineIndex] = offset;
  }

  /**
   * Try to parse the given field as a Number.
   * @param field The value to parse.
   * @return the Number value of the  field, or null if it couldn't be parsed as a Number.
   */
  ...
  static String dateFormats[] = {
    "EEE MMM d hh:mm:ss z yyyy",	// Mon Sep 24 11:53:19 CDT 2001 
  ...
  private synchronized String readRow(int rowIndex) throws IOException {
    return readLine(getRowOffset(rowIndex));
  }

  private synchronized String readLine(int lineNum) throws IOException {
    return readLine(getLineOffset(lineNum));
  }

  private synchronized String readLine(long index) throws IOException {
    raf.seek(index);
    return raf.readLine();
  }
    ...
    try {
      ...
      if (vals != null) {
        for (int c = 0; c < vals.length; c++) {
          Object obj = vals[c];
          if (vals[c] != null && colClass != null && c < colClass.length) {
            if (java.lang.Number.class.isAssignableFrom(colClass[c])) {
              obj = getNumber(vals[c]);
            } else if (java.util.Date.class.isAssignableFrom(colClass[c])) {
              obj = getDate(vals[c]);
            }
          }
          if (c < rv.size()) {
      ...
      if (t instanceof NoSuchMethodError || 
          t instanceof NoSuchMethodException) { // pre J2SE1.4 do it the hard way
        ...
        for (int i = 0; i < len; c++) {
          ...
          Object obj = field;
          if (field != null && colClass != null && c < colClass.length) {
            if (java.lang.Number.class.isAssignableFrom(colClass[c])) {
              obj = getNumber(field);
            } else if (java.util.Date.class.isAssignableFrom(colClass[c])) {
              obj = getDate(field);
  ...
  public static String guessFieldSeparator(String[] lines) {
    /*
       most likely:  TAB comma space semicolon colon pipe
       need to check for quoted string fields
       regex for quoted text field
           fieldSep|^ "[\"     \""
     */
    String sep = "	"; // TAB
    ...
    if (sepv.size() > 0) {
      if (sepv.size() > 1) {
        ...
        for (int i = v.size()-1; i >= 0; i--) {
          if (Character.isLetterOrDigit( ((String)v.get(i)).charAt(0)) ) {
            v.remove(i);
          }
        } 
        for (int i = v.size()-1; i > 0; i--) {
  ...
  private synchronized Vector getRow(int rowIndex) throws IOException {
    /*
     * This is synchronized because the size is set by parseRow before 
     * the values are filled in.  This would permit another thread to 
     * access the row, between the time the size is set and the values inserted.
     */
...
//System.err.println("prsRow " + rowIndex + "\t" + rv.size()+ "\n");
  ...
  public Object getValueAt(int rowIndex, int columnIndex) 
    ...
    try {
      ...
      System.err.println(this + ".getValueAt("+rowIndex+","+columnIndex+") " + ioex);
    }
    return null;
  }
  ...
  public Class getColumnClass(int columnIndex) {
    ...
    return columnClass != null && columnIndex < columnClass.length 
           ? columnClass[columnIndex] : java.lang.Object.class;
  }

  public String getColumnName(int columnIndex) {
    if (columnName != null && columnIndex >= 0 && columnIndex < columnName.length) {
