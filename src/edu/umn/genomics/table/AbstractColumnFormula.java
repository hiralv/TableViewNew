/*
 * @(#) $RCSfile: AbstractColumnFormula.java,v $ $Revision: 1.2 $ $Date: 2004/08/18 17:57:12 $ $Name: TableView1_3 $
 *
 ...
 * 
 * Extending classes need to implement the method:
 *   calculateValueAt(int rowIndex, int columnIndex)
 * 
 * A formula is interpreted as java code.  
 ...
 *  <LI><b> Cells </b> - the Cells class that has a number of useful static methods </LI>
 * </UL>
 ...
 * 
 * @author       J Johnson
 * @version $Revision: 1.2 $ $Date: 2004/08/18 17:57:12 $  $Name: TableView1_3 $ 
 * @since        1.0
...
public abstract class AbstractColumnFormula implements Serializable, TableModelFormula {
   ...
   * e.g. "edu.umn.genomics.table.Cells".
   * @return a List of java class names the Intrepreter should import.
   */
  ...
  /**
   * Returns the value for the table cell at row and column indices.
   * This is synchronized to prevent clashes in detecting recursion
   * while evauting formulas.
   * @param rowIndex The table row of the cell.
   * @param columnIndex The table column of the cell.
  ...
  protected Object retrieveValueAt(int rowIndex, int columnIndex) {
    ...
    try {
      ...
      if (path.contains(cell)) {
        result = new Exception("Recursion evaluating cell " + path.get(0) + " at " + cell);  // recursion
      } else {
