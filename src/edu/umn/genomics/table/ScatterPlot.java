/**
 * @(#) $RCSfile: ScatterPlot.java,v $ $Revision: 1.28 $ $Date: 2004/08/02 20:23:45 $ $Name: TableView1_3 $
 *
 ...
 * @author       J Johnson
 * @version $Revision: 1.28 $ $Date: 2004/08/02 20:23:45 $  $Name: TableView1_3 $ 
 * @since        1.0
 * @see ColumnMap
 */
public class ScatterPlot extends JPanel 
  ...
  IndexSelectColor indexSelectColor;
  IndexSelectDrawable indexSelectDrawable;
  SimpleGraph graph;
  LineDataModel lineModel;
  GraphSegments gs = new GraphSegments();
  GraphPoints gp;  // nonselected
  GraphPoints gps; // selected
  GraphRects  gr;
  ...
  public void setSelectColor(Color selectColor) {
    ...
    if (indexSelectColor != null) {
      indexSelectColor.setSelectColor(selectColor);
    }
  }
  ...
  public void setSelectDrawable(Drawable drawable) {
    if (drawable == null) return;  // Remove when classLoad works
    selectDrawable = drawable;
    if (indexSelectDrawable != null) {
      indexSelectDrawable.setSelectDrawable(selectDrawable);
    }
  }
  ...
  protected void displayLoc(Point p) {
  ...
  public ScatterPlot(ColumnMap xColumn, ColumnMap yColumn) {
    TableViewPreferences.setAttributes(this);
    ...
    gps = new GraphPoints();
    gps.setData(this);
    gps.setColor(defaultColor);
    gps.setDefaultDrawable(null);
    graph.addGraphItem(gps);
  ...
  public void setSelectionModel(ListSelectionModel newModel) {
    if (newModel != null && newModel != lsm) {
      ...
      if (indexSelectColor == null) {
        indexSelectColor = new IndexSelectColor(selectColor, null, lsm);
      } else {
        indexSelectColor.setSelectionModel(lsm);
      }
      if (indexSelectDrawable == null) {
        indexSelectDrawable = new IndexSelectDrawable(selectDrawable, null, lsm);
      } else {
        indexSelectDrawable.setSelectionModel(lsm);
      }
      if (gp != null) {
        gp.setIndexedColor(indexSelectColor);
        // gp.setIndexedDrawable(new IndexSelectDrawable(new DrawableX(), null, lsm));
      }
      if (gps != null) {
        gps.setIndexedColor(indexSelectColor);
        gps.setIndexedDrawable(indexSelectDrawable);
      }
    }
  }
  /**
