/*
 * @(#) $RCSfile: ParallelCoordinateTable.java,v $ $Revision: 1.3 $ $Date: 2004/01/28 21:37:15 $ $Name: TableView1_3 $
 *
 ...
 * @author       J Johnson
 * @version $Revision: 1.3 $ $Date: 2004/01/28 21:37:15 $  $Name: TableView1_3 $ 
 * @since        1.0
 * @see  ColumnMap
 * @see  TableContext
 ...
 */
public class ParallelCoordinateTable extends AbstractTableModelView 
  ...
  private void init() {
    ...
    tabPane.addTab("Row Graphs",null,graphPanel,"Display A Row Graph for each value of the Column.");
    tabPane.addTab("Graph Prototype",null,pcp,"Sets the appearance of the graph.");
  ...
  public void launch(String value, int[] rowIndices) {
    try {
      ParallelCoordinatePanel newPcp = (ParallelCoordinatePanel)pcp.clone();
  ...
  private void showTable() {
    if (tm != null) {
      ...
      if (colIndex >= 0 && colIndex < tm.getColumnCount()) {
        ...
        if (widths != null) {
          for (int i = 0; i < widths.length; i++) {
            jt.getTableHeader().setResizingColumn(jt.getColumnModel().getColumn(i));
            ...
            new MouseAdapter() {
              public void mouseClicked(MouseEvent e) {
                if (e.getClickCount() > 1) {
                  ...
                  int ci = tbl.convertColumnIndexToModel(tbl.columnAtPoint(e.getPoint()));
                  if (ci == 1) {
                    int ri = tbl.rowAtPoint(e.getPoint());
                    if (ri >= 0) {
                      int[] indices = (int[])tbl.getModel().getValueAt(ri,ci);
                      if (indices != null) {
                        Object obj = tbl.getModel().getValueAt(ri,0);
