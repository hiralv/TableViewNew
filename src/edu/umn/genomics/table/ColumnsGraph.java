/*
 * @(#) $RCSfile: ColumnsGraph.java,v $ $Revision: 1.5 $ $Date: 2003/08/06 17:20:52 $ $Name: TableView1_3 $
 *
 ...
 * as a line graph, with one line per column in the table.
 * The horizontal axis represents the range of rows in the table.
 * @author       J Johnson
 * @version $Revision: 1.5 $ $Date: 2003/08/06 17:20:52 $  $Name: TableView1_3 $ 
 * @since        1.0
 * @see  ColumnMap
 * @see  TableContext
 ...
 */
public class ColumnsGraph extends AbstractTableModelView
  ...
  private void setLabeler() {
    ...
    if (labelColumn >= 0 && labelColumn < ncols) {
      ...
      graph.getAxisDisplay(BorderLayout.NORTH).setAxisLabeler(new DecimalLabeler("###"));
    }
  }
  ...
  /**
   * Constructs a ColumnsGraph display.  Nothing will be displayed 
  ...
  /**
   * Constructs a ColumnsGraph diaplay which is initialized with 
...
/*
  private void setTicks(Graphics g) {
    ...
    double ppr = (xAxisDisp.getWidth() / (double)nrow);
    double tn = Math.ceil(Math.log(nrow) * 0.43429448190325176) * cw / 
                  (xAxisDisp.getWidth() / (double)nrow);
    double nti = ti;
...
//System.err.println("ti " + lw + " " + ppr + " " + lc + " " + tn + " " + nti + " " + exp);
  }
*/
  ...
  public void paintComponent(Graphics g) {
    ...
    if (!lsm.isSelectionEmpty()) {
      ...
      int xoff = xAxisDisp.getLocationOnScreen().x - getLocationOnScreen().x 
                 + xAxisDisp.getInsets().left;
      int h = getHeight() - insets.top - insets.bottom;
      int y = insets.top;
      //g.setColor(getBackground().brighter());
      Color sc = getBackground();
      g.setColor(new Color(sc.getRed()-20,sc.getGreen()-20,sc.getBlue()-20));
      ...
      for (int r = mn, a = mn; r <= mx; r++) {
        ...
        if (sel && (!rs || r == mx)) {
          int xl = xoff + xAxis.getIntPosition((double)a);
          if (a == r) {
            g.drawLine(xl,y,xl,y+h);
          } else {
            int xr = xoff + xAxis.getIntPosition((double)r);
    ...
    public Component getListCellRendererComponent(
      ...
      boolean cellHasFocus)    // the list and the cell have the focus
     {
        ...
        if (isSelected) {
          ...
          if (bg && color != null && index < color.length && color[index] != null)
          ...
          //if (color != null && index < color.length && color[index] != null)
            //setForeground(color[index]);
          //else 
