/*
 * @(#) $RCSfile: JTableView.java,v $ $Revision: 1.12 $ $Date: 2004/08/02 20:23:42 $ $Name: TableView1_3 $
 *
 ...
 *
 * @author       J Johnson
 * @version $Revision: 1.12 $ $Date: 2004/08/02 20:23:42 $  $Name: TableView1_3 $ 
 * @since        1.0
 ...
 */
  ...
  class BooleanRenderer extends JCheckBox implements TableCellRenderer
    {
        ...
        public Component getTableCellRendererComponent(JTable table, Object value,
            ...
            setEnabled(value != null);  // Added from the JTable BooleanRenderer version 
            return this;
        }
    }
  ...
  /** a virtual  model on the real table so that we can sort rows or add columns. */
  ...
  /** Maps the row selections between the tm and vtm row SelectionModels lsm and tlsm. */
  ...
  TableModelListener tml = new TableModelListener() {
      public void tableChanged(TableModelEvent e) {
        if (e.getSource() != null) {
          try {
          // System.err.println("TMEvt\t" +  vtm.getTableModel().getRowCount() + "\t" + e.getType() + " : " + e.getFirstRow() + " - " + e.getLastRow() + e.getSource());
  ...
  private void init() {
    ...
    if (table == null) {
      ...
      table.setDefaultRenderer(java.lang.Number.class, new NumberRenderer());
      table.setDefaultRenderer(java.lang.Double.class, new NumberRenderer());
      table.setDefaultRenderer(java.lang.Float.class, new NumberRenderer());
      table.setDefaultRenderer(java.lang.Integer.class, new NumberRenderer());
      table.setDefaultRenderer(java.lang.Boolean.class, new BooleanRenderer());
      //
      ...
      if (true) {  // Cells now handles sorting for JDK1.1 too.
        ...
        MouseInputAdapter headerListener = 
          new MouseInputAdapter() {
            Cursor defaultCursor = table.getTableHeader().getCursor();
            private int columnToSort(MouseEvent e) {
              JTableHeader header = table.getTableHeader();
              int colIdx = header.columnAtPoint(e.getPoint());
              if (colIdx >= 0) {
                Rectangle rect = header.getHeaderRect(colIdx);
                TableColumn aColumn = header.getColumnModel().getColumn(colIdx);
                ...
                Component component = renderer.getTableCellRendererComponent(table,
                                                  ...
                                                  -1, colIdx);
                int left = component instanceof JComponent ? ((JComponent)component).getInsets().left : 0;
                if (e.getX() < rect.x + left + sortArrow.getIconWidth()) {
            ...
            public void mouseExited(MouseEvent e) {
              table.getTableHeader().setCursor(defaultCursor);
            }
            ...
            public void mouseMoved(MouseEvent e) {
              ...
              if (column >= 0) {
                // sort cursor
                if (isAscending(e)) {
                  table.getTableHeader().setCursor(Cursor.getPredefinedCursor(Cursor.N_RESIZE_CURSOR));
                } else {
                  table.getTableHeader().setCursor(Cursor.getPredefinedCursor(Cursor.S_RESIZE_CURSOR));
                }
              } else {
                // default cursor
                table.getTableHeader().setCursor(defaultCursor);
    ...
    rowNumSortLbl.addMouseListener(
        new MouseAdapter() {
          public void mouseClicked(MouseEvent e) {
            if (e.isControlDown()) {
              removeSortColumn(Cells.ROWNUM_ASCENDING);
            } else {
              boolean ascending = e.getY() <= ((Component)e.getSource()).getSize().height / 2
                          ? true : false;
              setSortColumns(ascending?Cells.ROWNUM_ASCENDING:Cells.ROWNUM_DESCENDING, e.isShiftDown());
            }
          }
        }
    );
...
//    toolbar.add(jcbx);
    ...
    final JLabel lsvSort = new JLabel(new RowSortArrow(Cells.SELECTED_ROWS_FIRST,Cells.SELECTED_ROWS_LAST));
    lsvSort.setToolTipText("Sort by rows selected");
    lsvSort.setBorder(UIManager.getBorder("TableHeader.cellBorder"));
    lsvSort.addMouseListener(
        new MouseAdapter() {
          public void mouseClicked(MouseEvent e) {
            if (e.isControlDown()) {
              ...
              boolean ascending = e.getY() <= ((Component)e.getSource()).getSize().height / 2
                          ? true : false;
              setSortColumns(ascending?Cells.SELECTED_ROWS_FIRST:Cells.SELECTED_ROWS_LAST, e.isShiftDown());
            }
          }
        }
    );
    table.getTableHeader().addComponentListener(
      new ComponentAdapter() {
        public void componentResized(ComponentEvent e) {
          lsvSort.setPreferredSize(new Dimension(
              sortArrow.getIconWidth(), 
              Math.max(sortArrow.getIconHeight(), table.getTableHeader().getPreferredSize().height)));
        }
      }
    );
  ...
  private synchronized void removeSortColumn(int column) {
    if (sortColumns != null && sortColumns.length > 0) {
      int ciAsc = column >= 0 ? column : column == Cells.ROWNUM_DESCENDING ? Cells.ROWNUM_ASCENDING : -column-1;
      int ciDesc = column < 0 ? column : column == Cells.ROWNUM_ASCENDING ? Cells.ROWNUM_DESCENDING : -column-1;
        ...
        if (sortColumns[i] == ciAsc || sortColumns[i] == ciDesc) {
          if (sortColumns.length > 1) {
            ...
            if (sortColumns.length > i) {
              System.arraycopy(tmp,i+1,sortColumns,i,sortColumns.length - i);
            }
  ...
  private synchronized void setSortColumns(int column, boolean append) {
    ...
    if (append && sortColumns != null && sortColumns.length > 0) {
      ...
      if (sortColumns != null && sortColumns.length == 1 && sortColumns[0] == column) {
        if (column != Cells.SELECTED_ROWS_FIRST && column != Cells.SELECTED_ROWS_LAST) { 
          sortNeeded = false;
        }
      } else {
   ...
   * @param selectedFirst If true, sort selected rows first, else last.
   */
  ...
  /**
   * Sort rows according by the columns in the column array.
   * The column indices are interpretted as:
   *   index >= 0 sort on this column in ascending order
   *   index <  0 sort on column -index-1 in descending order
   *   index == Cells.ROWNUM_ASCENDING sort by table row order ascending
   *   index == Cells.ROWNUM_DESCENDING sort by table row order descending
   *   index == Cells.SELECTED_ROWS_FIRST sort selected table rows first
   *   index == Cells.SELECTED_ROWS_LAST sort selected table rows last
   * If column is null, don't sort the table.
   * If column.length == 0, sort in reverse table order.
   * @param column An array of column indices which define the sort order.
   */
  private void sortOnColumn(int[] column) {
    ...
    try {
      if (column == null || (column.length > 0 && column[0] == Cells.ROWNUM_ASCENDING)) {
      } else if (column.length < 1 || (column.length > 0 && column[0] == Cells.ROWNUM_DESCENDING)) {
        int si[] = new int[vtm.getRowCount()];
        ...
        if (ci < vtm.getColumnCount() && vtm.getColumnList().getElementAt(ci) instanceof RowNumColumn) {
          if (column[0] < 0) {
            int si[] = new int[vtm.getRowCount()];
            for (int i = 0, j = si.length-1; i < si.length; i++, j--) {
              si[i] = j;
  ...
  class RowSortArrow extends SortArrow {
    ...
    public void paintIcon(Component c, Graphics g, int x, int y) {
      ...
      if (sortColumns == null && ascVal == Cells.ROWNUM_ASCENDING) {
        filled = true;
        asc = true;
      } else if (sortColumns != null && sortColumns.length < 1 && dscVal == Cells.ROWNUM_DESCENDING) {
        filled = true;
        asc = false;
