/*
 * @(#) $RCSfile: CellMap.java,v $ $Revision: 1.6 $ $Date: 2004/08/18 17:32:54 $ $Name: TableView1_3 $
 *
 ...
 * If the common Class type of the Collection is Number or Date, 
 * or if all of the * values of the Collection are Strings that can be parsed 
 ...
 * from the Collection and indicate the selected indices in a ListSelectionModel.  
 * A new selection is be specified as a subrange of values between the
 * the minimum and maximum values of the collection.
 * New selections may be combined with the previous selections using standard 
 * set operators.
 * 
 * @author       J Johnson
 * @version $Revision: 1.6 $ $Date: 2004/08/18 17:32:54 $  $Name: TableView1_3 $ 
 * @since        1.0
 * @see  SetOperator 
...
public interface CellMap {
  /** Sort the cells by their natural order, 
   * e.g. alphabetically for text strings, numerically for numbers. 
   */
  public static final int NATURALSORT = 0;
  /** Sort the cells by comparing their respective alphabetic and numeric parts,
   * e.g. page2b, page10a,  page10c.
   */
  ...
  /** The TableModel changed and this map is no longer valid for this TableModel. 
   */
   ...
   * @param sortOrder the sorting order for mapping this collection.
   */
  ...
  /**
   * Return the mapped value for each element.
   * return the mapped value for each element.
   */
   ...
   * @param index the index into the list of elements being mapped.
   * return the mapped value of the element at the given index. 
   */
  public double getMapValue(int index);

  /**
   * For mapped set elements, return the element that is mapped nearest 
   * to the the mapValue in the given direction. For Numbers or Dates 
   * mapped along a continuum, return the value (Number or Date) that the 
   * given mapPosition represents along the continuum (dir is ignored).
   * @param mapValue the relative position on the map from minimum to maximum.
   * @param dir negative means round down, positive mean round up, 
   *            0 rounds to closest element.
   ...
   * in each subrange along the CellMap.
   ...
   * 10 if the collection is numeric, else the number of discrete elements.
   ...
   * @return true if values in this collection are mapped to a continuous range.
   */
  public boolean isContinuous();

  /** 
   * Return whether all values in this collection are number values.
   * @return true if all values in this collection are number values, else false.
   */
