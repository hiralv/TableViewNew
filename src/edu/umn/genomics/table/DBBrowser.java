/*
 * @(#) $RCSfile: DBBrowser.java,v $ $Revision: 1.15 $ $Date: 2004/08/02 20:23:39 $ $Name: TableView1_3 $
 *
  ...
  private void setPrimaryKeyInfo(Object[] tables) {
    ...
    if (tables != null && tables.length > 0) {
      ...
      for (int i = 0; i < cl.length; i++) {
        try {
          ResultSet rs = dbmd.getPrimaryKeys((String)cl[i], null,null);
          if (n < 1) {
            pKeyModel.set(rs);
          } else {
            pKeyModel.add(rs);
      ...
      for (int i = 0; i < sl.length; i++) {
        try {
          ResultSet rs = dbmd.getPrimaryKeys(null,(String)sl[i], null);
          if (n < 1) {
            pKeyModel.set(rs);
          } else {
            pKeyModel.add(rs);
  ...
  private void setForeignKeyInfo(Object[] tables) {
    ...
    if (tables != null && tables.length > 0) {
      for (int i = 0; i < tables.length; i++) {
        DBTable tbl = (DBTable)tables[i];
        try {
          ResultSet rs = dbmd.getImportedKeys(tbl.getCatalogName(), tbl.getSchemaName(),tbl.getTableName());
          if (i < 1) {
            fKeyModel.set(rs);
          } else {
            fKeyModel.add(rs);
      ...
      for (int i = 0; i < cl.length; i++) {
        try {
          ResultSet rs = dbmd.getImportedKeys((String)cl[i], null,null);
          if (n < 1) {
            fKeyModel.set(rs);
          } else {
            fKeyModel.add(rs);
      ...
      for (int i = 0; i < sl.length; i++) {
        try {
          ResultSet rs = dbmd.getImportedKeys(null,(String)sl[i], null);
          if (n < 1) {
            fKeyModel.set(rs);
          } else {
            fKeyModel.add(rs);
  ...
  private void setExportedKeyInfo(Object[] tables) {
    ...
    if (tables != null && tables.length > 0) {
      for (int i = 0; i < tables.length; i++) {
        DBTable tbl = (DBTable)tables[i];
        try {
          ResultSet rs = dbmd.getExportedKeys(tbl.getCatalogName(), tbl.getSchemaName(),tbl.getTableName());
          if (i < 1) {
            xKeyModel.set(rs);
          } else {
            xKeyModel.add(rs);
      ...
      for (int i = 0; i < cl.length; i++) {
        try {
          ResultSet rs = dbmd.getExportedKeys((String)cl[i], null,null);
          if (n < 1) {
            xKeyModel.set(rs);
          } else {
            xKeyModel.add(rs);
      ...
      for (int i = 0; i < sl.length; i++) {
        try {
          ResultSet rs = dbmd.getExportedKeys(null,(String)sl[i], null);
          if (n < 1) {
            xKeyModel.set(rs);
          } else {
            xKeyModel.add(rs);
...
System.err.println(">>> setIndexInfo");
    try {
        TableModel itm = AboutDB.getTableModel(dbmd.getIndexInfo(dbTable.getCatalogName(), dbTable.getSchemaName(), dbTable.getTableName(), false, true));
        if (cols != null && cols.size() == 0) {
          cols.add("TABLE_CAT");
...
//System.err.println("<<< setRows");
  ...
  private DBUser getDBParams() {
    ...
    if (dbprops != null) {
      dbuser = new DBUser(dbprops.getProperty("account"),
  ...
  public DBBrowser() {
    ...
    stopBtn.addActionListener(
      new ActionListener() {
        public void actionPerformed(ActionEvent e) {
          ...
          if (rowModel != null) {
            DoSpinner.setValue(pageSize,new Integer(rowModel.getRowCount()));
          }
    ...
    tableViewBtn.addActionListener(
      new ActionListener() {
        public void actionPerformed(ActionEvent e) {
          launchTableView();
        }
    ...
    cPnl.addTab("Imported Foreign Key", null, fKeyjsp, "Declared Imported Foreign Keys");
    cPnl.addTab("Exported Foreign Key", null, xKeyjsp, "Declared Exported Foreign Keys");
    ...
    ((JSpinner)curPage).addChangeListener( 
      new ChangeListener() {
