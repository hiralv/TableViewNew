/*
 * @(#) $RCSfile: DefaultColumnMap.java,v $ $Revision: 1.12 $ $Date: 2004/08/18 17:32:55 $ $Name: TableView1_3 $
 *
 ...
 * 
 * @author       J Johnson
 * @version $Revision: 1.12 $ $Date: 2004/08/18 17:32:55 $  $Name: TableView1_3 $ 
 * @since        1.0
 * @see  SetOperator 
 * @see  javax.swing.table.TableModel 
 * @see  javax.swing.ListSelectionModel
 */
public class DefaultColumnMap extends BaseColumnMap 
   ...
   * @return the number of rows that are boolean values, or -1 if there is a non-boolean.
   */
  ...
  private synchronized void mapTheColumn() {
    ...
    if (tm != null && tm.getRowCount() > 0) {
      ...
      if (nums < 0) {         // at least one not null value was not a number
        ...
        isNumber = false;
        isDate = false;
        mapObject(nullTag); // create a single NULL entry and then adjust count
        try {
      ...
      // System.err.println("mapTheColumn " + colIndex + "\tdone " + (t1-t0));
  ...
  private void setMinMax() {
    ...
    if (dv != null && dv.length > 0) {
      ...
      // System.err.println("setMinMax " + min + " " + max + " \t" + mn + " " + mx);
      setMin(mn);
      setMax(mx);
    }
  }
  ...
  public void tableChanged(TableModelEvent e) {
    if (e == null || e.getFirstRow() == TableModelEvent.HEADER_ROW) {
      ...
      // System.err.println("\n" + colIndex + "HEADER_ROW ");
      if (e instanceof TableModelEventExt) {
        TableModelEventExt evt = (TableModelEventExt)e;
        if (evt.isColumnEvent()) {
          int idx = evt.getNewColumnIndex(colIndex);
System.err.println("DefaultColumnMap ci " + colIndex + " -> " + idx);
          if (idx >= 0) {
            colIndex = idx;
            return;
          }
          ...
          if ( (isDate && (parseDates(rs,re,nv) >= 0))  || 
            ...
            if (dv != null) {
              ...
              if (dvals == null && rs == 0) {
                ...
                double tmp[] = dvals;
                // allocate array
                dvals = new double[(re - rs + 1) + (tmp != null ? tmp.length : 0)];
                if (tmp != null) {
                  // copy part before rs
                  System.arraycopy(tmp, 0, dvals, 0, rs);
                  // copy part after re
                  System.arraycopy(tmp, rs, dvals, re+1, tmp.length - rs);
                }
                // copy insert
                System.arraycopy(dv, 0, dvals, rs, re - rs + 1);
              }
      ...
      case TableModelEvent.DELETE:
        ...
        if (colTyped && (isNumber || isDate)) {
          ...
          if (dv != null) {
            ...
            rowCount = dvals.length;
            if (rs > 0) {
              // System.err.println("System.arraycopy("+ dv + ",0,"+dvals+",0,"+rs+")" );
              System.arraycopy(dv, 0, dvals, 0, rs);
            }
            if (re < dv.length-1) {
              System.arraycopy(dv, re+1, dvals, rs, dv.length - 1 - re);
            }
            setMinMax();
            statsNeeded();
            setState(CellMap.MAPPED);
