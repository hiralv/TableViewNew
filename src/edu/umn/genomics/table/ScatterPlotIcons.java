/*
 * @(#) $RCSfile: ScatterPlotIcons.java,v $ $Revision: 1.3 $ $Date: 2004/05/13 14:42:45 $ $Name: TableView1_3 $
 *
...
/**
 * Display scatterplots of the values of pairs of columns of a table.
 * Each pair of columns is represented by a checkbox button containing 
 * an icon of the scatterplot for the pair.  
 * Selecting the button displays the scatterplot in the drawing region 
 * of the panel.  
 * Deselecting the button removes the scatterplot from the drawing region. 
 *
 * @author       J Johnson
 * @version $Revision: 1.3 $ $Date: 2004/05/13 14:42:45 $  $Name: TableView1_3 $ 
 * @since        1.0
 * @see  ColumnMap
 * @see  TableContext
 ...
 */
public class ScatterPlotIcons extends AbstractTableModelView 
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (ia != null && ia.length >= 2) {
          try {
            ...
            ScatterPlotView view = (ScatterPlotView)ctx.getView(tm,viewName);
  ...
  /** 
   * Create an ScatterPlotIcons Panel with an icon for each pair of 
   * columns in the TableModel.  The data in the TableModel will be mapped 
   * by ColumnMaps provided by the TableContext.
   * The ScatterPlotIcons uses the ListSelectionModel from the TableContext.
   * @param tableModel the data. 
   */
   ...
   * columns in the TableModel.  The data in the TableModel will be mapped 
   * by ColumnMaps provided by the TableContext.
   * The ScatterPlotIcons uses the given ListSelectionModel.
   * @param tableModel the data. 
   * @param lsm the row selection model for the data table. 
   */
    ...
    if (tm != null) {
      ...
      for (int r = 0; r < ncols; r++) {
        ...
        for (int c = 0; c < ncols; c++) {
          ...
          b.setBorder(BorderFactory.createLineBorder(Color.lightGray,bw));
        ...
        JLabel lbl;
        String tip = tm.getColumnName(r) + "  mean: " + ctx.getColumnMap(tm,r).getAvg() 
                                         + " stddev: " + ctx.getColumnMap(tm,r).getStdDev();
        lbl = new LeftLabel(tm.getColumnName(r),b.getPreferredSize().height);
        lbl.setToolTipText(tip);
        leftPanel.add(lbl);
        lbl = new TopLabel(tm.getColumnName(r),b.getPreferredSize().width);
        lbl.setToolTipText(tip);
        topPanel.add(lbl);
      }
  ...
  private boolean updateIcon(JButton b) {
    ...
    if (cm1.getState() != CellMap.MAPPED || cm1.getState() != CellMap.MAPPED) 
      return false;
    b.setIcon(makeIcon(cm1, cm2,iw,ih));
    b.setMargin(new Insets(0,0,0,0));
    float corr = (float)cm1.correlation(cm2);
    float blue = Float.isNaN(corr)||Float.isInfinite(corr) ? 0f :(corr + 1f) / 2f; 
    blue = blue < 0f ? 0f : blue > 1f ? 1f : blue;
    float red = Float.isNaN(corr)||Float.isInfinite(corr) ? 0f :1f - blue;  
    red = red < 0f ? 0f : red > 1f ? 1f : red;
    float green = Float.isNaN(corr)||Float.isInfinite(corr) ? 0f :.5f - Math.abs(corr)/2f;
    green = green < 0f ? 0f : green > 1f ? 1f : green;
    b.setBorder(BorderFactory.createLineBorder(new Color(red,green,blue),bw));
    b.setToolTipText(tm.getColumnName(r) + "," + tm.getColumnName(c) + " correlation: " + corr);
    return true;
  }
   ...
   * @param xColumn data column for the x axis value
   * @param yColumn data column for the y axis value
   * @param w the width for the icon
  ...
  public Icon makeIcon(ColumnMap xColumn, ColumnMap yColumn, int w, int h) {
    ...
    if (xColumn != null && yColumn != null) {
      ...
      LinearAxis xAxis = new LinearAxis(); // axes to map x values to pixel offsett
      LinearAxis yAxis = new LinearAxis(); // axes to map y values to pixel offsett
...
/*
class TopLabel extends JComponent {
  int pw = 20;
  String text = "hello";
  public TopLabel(String text, int w) {
    //super(text);
  ...
  public void paintComponent(Graphics g) {
    ...
    g.drawLine(2,getWidth() / 2,0 , getWidth() / 2);
      Graphics2D g2 = (Graphics2D)g;
      g2.setColor(Color.black);
    g.drawString(getText(),getHeight()-2,getWidth() / 2);
    g.drawLine(getHeight()-2,getWidth() / 2,0 , getWidth() / 2);
      AffineTransform saveXform = g2.getTransform();
      AffineTransform rotXform = new AffineTransform();
      rotXform.rotate(Math.toRadians(90));
      g2.setTransform(rotXform);
      //super.paintComponent(g2);
      g2.setColor(Color.orange);
    g.drawLine(getHeight()-2,getWidth() / 2,0 , getWidth() / 2);
      g2.setColor(getForeground());
      g2.setColor(Color.black);
      g2.drawString(getText(),10,10);
  ...
  public Dimension getPreferredSize() {
    ...
    dim.height = fm.stringWidth(getText());
    dim.width = pw;
    return dim;
  }
}
