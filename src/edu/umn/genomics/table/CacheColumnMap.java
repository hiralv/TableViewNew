...
public class CacheColumnMap extends BaseColumnMap {
  ...
  public Class getColumnClass() {
    if (colClass != null && colClass == java.lang.Number.class && data != null) {
 ...
 // CellMap
  ...
  public Object getMappedValue(double mapValue, int dir) {
    if (data != null && data instanceof BitSet) {
      return Math.abs(FALSE_VALUE - mapValue) < Math.abs(TRUE_VALUE - mapValue) 
               ? new Boolean(false) 
               : new Boolean(true);
    } else if (isNumber) {
      if (data == null) {
  ...
  public double getMapValue(int rowIndex) {
    ...
    if (data != null && data instanceof BitSet) {
      ...
      return obj instanceof Boolean ? ((Boolean)obj).booleanValue() == true ? TRUE_VALUE : FALSE_VALUE : NULL_VALUE;
    } else if (isNumber) {
      if (data == null || rowIndex >= Array.getLength(data)) {
        ...
        return (double)(((double[])data)[rowIndex]);
      } else if (data.getClass().getComponentType() == java.lang.Long.TYPE) {
        return (double)(((long[])data)[rowIndex]);
      } else if (data.getClass().getComponentType() == java.lang.Integer.TYPE) {
        return (double)(((int[])data)[rowIndex]);
      } else if (data.getClass().getComponentType() == java.lang.Float.TYPE) {
        return (double)(((float[])data)[rowIndex]);
      } else if (data.getClass().getComponentType() == java.lang.Short.TYPE) {
        return (double)(((short[])data)[rowIndex]);
      } else if (data.getClass().getComponentType() == java.lang.Byte.TYPE) {
        return (double)(((byte[])data)[rowIndex]);
      }
    } else if (isDate) {
      if (data == null || rowIndex >= Array.getLength(data)) {
        Object obj = getValueAt(rowIndex);
      ...
      return ((long[])data)[rowIndex] != Long.MAX_VALUE 
               ? (double)(((long[])data)[rowIndex]) : NULL_VALUE;
    } else {
      int i = getMappedIndex(rowIndex);  
      if (sortIndex != null) {
        // System.err.println( " gmv " + rowIndex +"\t" + i +"\t"+sortIndex.getDst(i));
      ...
      if (data instanceof BitSet) {
        ...
        if (nullCells != null) {
          for (int i = 0; i < dm.length; i++) 
            dm[i] = nullCells.get(i) ? NULL_VALUE : bs.get(i) ? TRUE_VALUE : FALSE_VALUE;
        } else {
          for (int i = 0; i < dm.length; i++) 
        ...
        if (data.getClass().getComponentType() == java.lang.Double.TYPE) {
          ...
          System.arraycopy((double[])da, 0, dm, 0, dm.length);
        } else if (data.getClass().getComponentType() == java.lang.Float.TYPE) {
  ...
  public Object getValueAt(int rowIndex) {
    if (data != null) {
      if (data instanceof BitSet) {
        ...
        return rowIndex >= 0 ? new Boolean(((BitSet)data).get(rowIndex)) : null;
      } else if (isNumber) {
        if (nullCells != null && nullCells.get(rowIndex)) {
          return null;
        }
        return Array.getLength(data) > rowIndex
               ? Array.get(data, rowIndex)
               : null;
      } else if (isDate) {
        return ((long[])data).length > rowIndex 
                  && ((long[])data)[rowIndex] != Long.MAX_VALUE 
               ? new Date(((long[])data)[rowIndex]) 
               : null;
      }
      // Object
  ...
  protected Object checkArraySize(Object dataArray, int size) {
    ...
    int oldLength = array != null ? java.lang.reflect.Array.getLength(array) : 0; 
    if (oldLength < size) {
  ...
  protected Object setArraySize(Object dataArray, int size) {
    ...
    int oldLength = array != null ? java.lang.reflect.Array.getLength(array) : 0; 
    if (oldLength != size) {
      Object tmp = java.lang.reflect.Array.newInstance(array.getClass().getComponentType(),size);
      if (size > oldLength) {
        if (array.getClass().getComponentType() == Double.TYPE) {
          Arrays.fill((double[])tmp,oldLength > 0 ? oldLength - 1 : 0, size-1, Double.NaN); 
        } else if (array.getClass().getComponentType() == Float.TYPE) {
          Arrays.fill((float[])tmp,oldLength > 0 ? oldLength - 1 : 0, size-1, Float.NaN); 
        }
      }
      System.arraycopy(array, 0, tmp, 0, oldLength < size ? oldLength : size);
      array = tmp;
    }
    } catch (Throwable t) {
  ...
  synchronized private void createDataArray(Class columnClass) {
    if (java.lang.Boolean.class.isAssignableFrom(columnClass)) {
      ...
      //System.err.println("COLUMN " + colIndex + " " + colClass + " " + data.getClass().getComponentType());
      return;
  ...
  private Object allocNumArray(Class columnClass, int size) {
    ...
    if (java.lang.Number.class.isAssignableFrom(columnClass)) {
      if (java.lang.Double.class == colClass) {
        dataArray = java.lang.reflect.Array.newInstance(Double.TYPE,size);
      } else if (java.lang.Long.class == colClass) {
        dataArray = java.lang.reflect.Array.newInstance(Long.TYPE,size);
      } else if (java.lang.Integer.class == colClass) {
        dataArray = java.lang.reflect.Array.newInstance(Integer.TYPE,size);
      } else if (java.lang.Float.class == colClass) {
        dataArray = java.lang.reflect.Array.newInstance(Float.TYPE,size);
      } else if (java.lang.Short.class == colClass) {
        dataArray = java.lang.reflect.Array.newInstance(Short.TYPE,size);
      } else if (java.lang.Byte.class == colClass) {
        dataArray = java.lang.reflect.Array.newInstance(Byte.TYPE,size);
  ...
  private void checkNumDataType(Number num, double dval, int ri) {
    ...
    if (data.getClass().getComponentType() == java.lang.Double.TYPE) {
      ...
      if (dval < (double)Float.MIN_VALUE || dval > (double)Float.MIN_VALUE) {
        float[] oldArr = (float[])data;
        double newArr[] = new double[oldArr.length]; 
        for (int i = 0; i < oldArr.length; i++) {
          newArr[i] = (double)oldArr[i];
        }
        data = newArr;
      }
    } else {
      boolean isFloat = num == null || Math.abs(dval - num.longValue()) > 0.;
      if (data.getClass().getComponentType() == java.lang.Long.TYPE) {
        if (isFloat) {
          long[] oldArr = (long[])data;
          double newArr[] = new double[oldArr.length]; 
          for (int i = 0; i < oldArr.length; i++) {
            newArr[i] = nullCells != null && nullCells.get(i) ? Double.NaN : (double)oldArr[i];
          }
          data = newArr;
          nullCells = null;
        }
        ...
        if (isFloat) {
          int[] oldArr = (int[])data;
          if (Double.isInfinite(dval) || dval < (double)Float.MIN_VALUE || dval > (double)Float.MIN_VALUE) {
            double newArr[] = new double[oldArr.length]; 
            for (int i = 0; i < oldArr.length; i++) {
              newArr[i] = nullCells != null && nullCells.get(i) ? Double.NaN : (double)oldArr[i];
            }
            ...
            for (int i = 0; i < oldArr.length; i++) {
              newArr[i] = nullCells != null && nullCells.get(i) ? Float.NaN : (float)oldArr[i];
            }
            data = newArr;
          }
          nullCells = null;
        } else if (dval >= Integer.MIN_VALUE && dval <= Integer.MAX_VALUE) {
          return;
        } else {
          int[] oldArr = (int[])data;
          // Convert to long
    ...
    //System.err.println("COLUMN " + colIndex + " " + colClass + "\t" + prevType + "\t -> " + data.getClass().getComponentType() + "\t" + ri);
  }
