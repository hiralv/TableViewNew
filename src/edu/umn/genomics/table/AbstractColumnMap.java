/*
 * @(#) $RCSfile: AbstractColumnMap.java,v $ $Revision: 1.11 $ $Date: 2004/08/18 17:32:54 $ $Name: TableView1_3 $
 *
 ...
 * The AbstractColumnMap can also be used to select a range of mapped values 
 ...
 * 
 * @author       J Johnson
 * @version $Revision: 1.11 $ $Date: 2004/08/18 17:32:54 $  $Name: TableView1_3 $ 
 * @since        1.0
 * @see  SetOperator 
...
public abstract class AbstractColumnMap implements Serializable, ColumnMap, CleanUp {
   ...
   * @param nullCount the number of null values in this column.
   */
   ...
   * @param infiniteCount the number of infinite values in this column.
   */
   ...
   * @param distinctCount the number of distinct values in this column.
   */
   ...
   * @param median the  median value in this column in this column.
   */
   ...
   * @param q1 the first quartile value in this column.
   */
   ...
   * @param q3 the third quartile value in this column.
   */
  ...
  /**
   * Set the mean avaerage value in this column in this column.
   * This is only meaningful for continuously mapped  columns.
   * @param avg the mean avaerage value in this column in this column.
   */
  ...
  /**
   * Set the variance of the values in this column in this column.
   * This is only meaningful for continuously mapped  columns.
   * @param variance the variance of the  values in this column in this column.
   */
   ...
   * This is only meaningful for continuously mapped  columns.
   * @param stdDev the  standard deviation of the values in this column in this column.
   */
   ...
   * @param  min the minimum mapped value of this column.
   */
   ...
   * @param max the maximum mapped value of this column.
   */
  ...
  /**
   * Return the element that is mapped nearest to the mapValue in the 
   * given direction.
   * @param mapValue the relative position on the map 
   * @param dir negative means round down, positive mean round up, 
   *            0 rounds to closest..
   * return the element that is mapped nearest to the mapValue.
   */
  public abstract Object getMappedValue(double mapValue, int dir) ;


  /**
   * Get a mapping from a sorted list of distinct values to the table rows that 
   * contain that value. 
   * PartitionIndexMap.getSrcs(0) will return the indices of the table rows that contain
   * that contain the first value in order.
  ...
  public boolean isNumber() {
    return isNumber || java.lang.Number.class.isAssignableFrom(tm.getColumnClass(colIndex));
  }
  ...
  public boolean isDate() {
    return isDate || java.util.Date.class.isAssignableFrom(tm.getColumnClass(colIndex));
  }
  ...
  public boolean isBoolean() {
    return isBoolean || java.lang.Boolean.class.isAssignableFrom(tm.getColumnClass(colIndex));
  }
...
//System.err.println(this + "\tselectRange\t" + from + "\t" + to);
  ...
  protected void collectStats(double[] dvals) {
    ...
    if (!isMinSet() || !isMinSet() || !isNullCountSet() || !isDistinctCountSet() || !isMedianSet()) {
   ...
   * @param map the other column
   * @param covariance the covariance value for the pair of columns.
   */
  ...
  /** 
   * For each value in the column return its distance from the mean 
  ...
  /** 
   * Get the regression line for the correlation of this column 
   * to the given column.
   * @param ymap the other column
  ...
  /** 
   * Get the regression line for the correlation of one column 
   * to the other column.
   * @param xmap a column
   * @param ymap the other column
   * @return the regression line for the pair of columns.
...
*/
  ...
  /**
   * Partition the column values text representation based on matches to the array of 
   * regular expression patterns. 
   * @param regex The array of regular expression patterns upon which to partition the values.
   * @return The PartitionIndexMap that partitions the values according to the regular expressions.
   */
  public PartitionIndexMap getRegexPartition(String[] regex) throws NullPointerException {
    return getRegexPartition(this,regex);
  }
  /**
   * Partition the column values text representation based on matches to the array of 
   * regular expression patterns. 
   * @param cmap The ColumnMap to partition
   * @param regex The array of regular expression patterns upon which to partition the values.
   * @return The PartitionIndexMap that partitions the values according to the regular expressions.
   */
  public static PartitionIndexMap getRegexPartition(ColumnMap cmap, String[] regex) 
  ...
  /**
   * Partition the column Date values based on a Calendar field.
   * @param calendarField The Calendar field upon which to partition the values.
   * @return The PartitionIndexMap that partitions the values according to the calendarField.
   */
   ...
   * @return The PartitionIndexMap that partitions the values according to the calendarField.
   */
  public static PartitionIndexMap getDatePartition(ColumnMap cmap, int calendarField) {
   ...
   * @param cmap The ColumnMap to partition
   * @param calendarField The Calendar field upon which to partition the values.
   * @param divisions The number of divisions of Calendar field values to group together, 
            e.g. Months divided into 4 groups yield Quarters of the year. 
   * @return The PartitionIndexMap that partitions the values according to the calendarField.
   */
  public static PartitionIndexMap getDatePartition(ColumnMap cmap, int calendarField, int divisions) 
      throws NullPointerException, IllegalArgumentException {
    if (cmap == null) {
      throw new NullPointerException("ColumnMap");
    }
    if (cmap.getColumnClass() != null && !java.util.Date.class.isAssignableFrom(cmap.getColumnClass())) {
    ...
    int div = divisions > 0 ? (cal.getActualMaximum(calendarField) - cal.getActualMinimum(calendarField) + 1) / divisions : 1;
    for (int ri = 0; ri < pmap.length; ri++) {
      Object obj = cmap.getValueAt(ri);
      int val = -1;
   ...
   * @param mapState the mapState of this map.
   */
