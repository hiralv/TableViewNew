/*
 * @(#) $RCSfile: MergeTableModel.java,v $ $Revision: 1.3 $ $Date: 2004/08/12 21:00:03 $ $Name: TableView1_3 $
 *
...
public class MergeTableModel extends AbstractTableModel  implements TableModelListener {
   ...
   * @exception IndexOutOfBoundsException If the column count of the tableModel 
   *         does not match the column count of the existing tables.
   * @exception ClassCastException If the column classes of the tableModel 
   *         do not match the column classes of the existing tables.
   */
  public void addTableModel(TableModel tableModel) 
      throws NullPointerException, IndexOutOfBoundsException, ClassCastException {
    // check for null
    if (tableModel == null) {
      throw new NullPointerException("Attempted to add a null TableModel to MergeTableModel");
    }
    if (getTableCount() > 0) {
  ...
  /**
   * Returns whether the cell in the referenced table at the given row and column is editable.
   *
   * @param   tableIndex  the index of the referenced table 
   * @param   rowIndex    the row whose value is to be queried
   * @param   columnIndex the column whose value is to be queried
   * @return  whether the cell in the table at the given row and column is editable.
   *  see #setValueAt
   */
  ...
  /**
   * Sets the value of the cell at the given row and column of the referenced table.
   *
   * @param   aValue      the new value for the cell
   * @param   tableIndex  the index of the referenced table 
   * @param   rowIndex    the row whose value is to be queried
   * @param   columnIndex the column whose value is to be queried
   *  see #isCellEditable(int,int,int)
   *  see #getValueAt(int,int,int)
   */
  ...
  /**
   * Gets the value of the cell at the given row and column of the referenced table.
   *
   * @param   tableIndex  the index of the referenced table 
   * @param   rowIndex    the row whose value is to be queried
   * @param   columnIndex the column whose value is to be queried
   * @return  The value of the cell in the referenced table at the given row and column.
   *  see #setValueAt(Object,int,int,int)
   */
  ...
  private boolean matchColumns(TableModel tm1, TableModel tm2) 
    ...
    if (tm1.getColumnCount() != tm2.getColumnCount()) {
      throw new IndexOutOfBoundsException("Tables have different number of columns: " +
               tm1.getColumnCount() + " != " + tm2.getColumnCount());
    }
    //column classes
    for (int ci = 0; ci < tm1.getColumnCount(); ci++) {
      if (java.lang.Number.class.isAssignableFrom(tm1.getColumnClass(ci)) && 
          java.lang.Number.class.isAssignableFrom(tm2.getColumnClass(ci))) {
        continue;
      }
      if (!tm1.getColumnClass(ci).isAssignableFrom(tm2.getColumnClass(ci))) {
        throw new ClassCastException("Table Columns " + ci + " don't match: " + 
                tm1.getColumnClass(ci) + " vs " + tm2.getColumnClass(ci));
      }
    }
    return true;
  }
  ...
  public Class getColumnClass(int columnIndex) {
    if (getTableCount() > 0) {
      if (columnIndex == 0) {
        ...
        Class colClass = getTableAt(0).getColumnClass(getTableCol(columnIndex));
        if (java.lang.Number.class.isAssignableFrom(colClass)) {
          for (int i = 1; i < getTableCount(); i++) {
            Class nextClass = getTableAt(i).getColumnClass(getTableCol(columnIndex));
