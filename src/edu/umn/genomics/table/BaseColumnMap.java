/*
 * @(#) $RCSfile: BaseColumnMap.java,v $ $Revision: 1.5 $ $Date: 2004/08/18 17:32:54 $ $Name: TableView1_3 $
 *
 ...
 * The BaseColumnMap can also be used to select a range of mapped values 
 ...
 * 
 * @author       J Johnson
 * @version $Revision: 1.5 $ $Date: 2004/08/18 17:32:54 $  $Name: TableView1_3 $ 
 * @since        1.0
 * @see  SetOperator 
...
public class BaseColumnMap extends AbstractColumnMap 
  ...
  protected void collectStats(Hashtable colH) {
    ...
    for (int i = 0, c=0, nc = 0; i < objList.size(); i++,c = nc) {
      elem = (Elem)colH.get(objList.get(sortIndex != null ? sortIndex.getSrc(i) : i));
      nc = c + elem.count;
      if (c < _mv && nc + 1 > _mv) {
        if  (nc  > _mv) 
          _median = elem.index;
    ...
    try {
      ...
      if (isBoolean()) {
        ...
        if (obj == null) {
          ...
          return s.equalsIgnoreCase("true") ? TRUE_VALUE : s.equalsIgnoreCase("false") ? FALSE_VALUE : NULL_VALUE;
        }
      }
      Hashtable colH = colHash;
      if (colH != null) {  // discreet values 
          ...
          for (int r = 0; r < nrows; r++) {
            if (lsm.isSelectedIndex(r)) {
              ...
              if (obj != null) {
                int i = offset *
                  (sortIndex != null
                   ? sortIndex.getDst(((Elem)colH.get(obj)).index)
