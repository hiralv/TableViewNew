/*
 * @(#) $RCSfile: DBColumnMap.java,v $ $Revision: 1.5 $ $Date: 2004/08/02 20:23:39 $ $Name: TableView1_3 $
 *
...
public class DBColumnMap extends CacheColumnMap {
  int sqlType;
  int[] nanos = new int[0];
  public DBColumnMap(TableModel tm, ListSelectionModel lsm, String name, int colIndex, Class colClass, int sqlType) {
  ...
  /**
   * Return a java Class for a given SQL Type.  
   * This is for JDBC 1.1  only, for JDBC 1.2 and later use 
  ...
  synchronized private void createDataArray(int sqlType) {
    ...
    case java.sql.Types.TIME :
      data = java.lang.reflect.Array.newInstance(Long.TYPE,bufIncr);
      break;
    case java.sql.Types.TIMESTAMP :  // This should be an array of java.sql.Types.TIMESTAMP
      data = java.lang.reflect.Array.newInstance(Long.TYPE,bufIncr);
      break;
   ...
   *  @param rs - the ResultSet which must be at the row corresponding to rowIndexcurrent row
   *  @rowIndex - the row whose value is to be set
   */
  synchronized void setValueAt(ResultSet rs, int rowIndex) throws SQLException {
    ...
    case java.sql.Types.LONGVARCHAR :
      {
        ...
        try {
          ...
          Reader cStr = rs.getCharacterStream(colIndex+1);
          if (cStr != null) {
            BufferedReader rdr = new BufferedReader(cStr);
            for(int len = rdr.read(cbuf,0,cbuf.length); len >= 0; len = rdr.read(cbuf,0,cbuf.length)) {
    ...
    case java.sql.Types.CLOB :
      {
        ...
        try {
          ...
          if (clob != null) {
            ...
            if (clen <= cbuflen) {
              val = clob.getSubString(1, (int)clen);  // first character is at position 1
            } else {
              char[] cbuf = new char[4096];
              StringBuffer sb = new StringBuffer();
              BufferedReader rdr = new BufferedReader(clob.getCharacterStream());
              for(int len = rdr.read(cbuf,0,cbuf.length); len >= 0; len = rdr.read(cbuf,0,cbuf.length)) {
                sb.append(cbuf,0,len);
              }
              val = sb.toString();
    ...
    default:
      if (System.getProperty("java.specification.version").compareTo("1.4")>=0) {
       ...
       // System.err.print(" NEED TO DEFINE " + sqlType + " for column " + colIndex);
      break;
    }
  }
