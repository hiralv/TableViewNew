/*
 * @(#) $RCSfile: DataViewJ3D.java,v $ $Revision: 1.2 $ $Date: 2004/01/29 15:53:21 $ $Name: TableView1_2 $
 *
...
/**
 * @author       J Johnson
 * @version $Revision: 1.2 $ $Date: 2004/01/29 15:53:21 $  $Name: TableView1_2 $
 * @since        1.0
 */
...
*/
   ...
   * @param name The name by which the data will be listed.
   * @param tableModel to display
...
//System.err.println("DV3D addDataMap " + dm.getName());
    ...
    try {
      ...
      if (!bbox.isEmpty()) {
        ...
        bbox.getLower(l);
        bbox.getUpper(u);
//System.err.println("[" + l.x + "," + l.y + "," + l.z + "] [" +  u.x + "," + u.y + "," + u.z + "]" + " " + bbox.isEmpty());
...
//System.err.println("DV3D addDataMap " + dm.getName());
    ...
    dm.doSelection(getViewAction() == NAVIGATE ? null : canvas, setOperator);
    return dm;
  }
...
//System.err.println("DV3D showDataMap " + dataMap.getName() + " " + flag);
  }
...
//System.err.println("setViewpoint center: " + center.toString() + "radius: " + radius + " " + sceneBounds.isEmpty());
...
//System.err.println("setViewpoint \n" + viewTrans.toString());
  ...
  /**
   * Set viewing behavior.
   * Mouse Button1 rotates the geometry.
   * Mouse Button1 zooms the geometry.
   * Mouse Button1 translates the geometry.
   */
   ...
   * Creates the axes.
   */
  public void setAxes( double xdl, double ydl, double zdl,
                       double xdh, double ydh, double zdh) {
    ...
    if (displayAxes) {
      ...
      if (true) {
        ...
        if (true) {
          ...
          LineArray la = new  LineArray(coord.length, GeometryArray.COORDINATES | GeometryArray.COLOR_3);
          la.setCoordinates(0,coord);
          la.setColors(0,color);
          Appearance appearance = new Appearance();
          LineAttributes lineAttributes = new LineAttributes(2.f, LineAttributes.PATTERN_SOLID, true);
        ...
        if (true) {
          ...
          PointArray pa = new  PointArray(tcnt, GeometryArray.COORDINATES);
          pa.setCoordinates(0,coord);
          Appearance appearance = new Appearance();
          PointAttributes pointAttributes = new PointAttributes(3.f, true);
        ...
        if (true) {
          Point3d coord[] = {
            lbb,  lbf,  rbf, rbb // around base
          };
          QuadArray qa = new QuadArray(coord.length, GeometryArray.COORDINATES|GeometryArray.NORMALS);
      ...
      if (!true) { // x axis
        ...
        LineArray xla = new  LineArray(vcnt, GeometryArray.COORDINATES | GeometryArray.COLOR_3);
        ...
        // LineAttributes lineAttributes = new LineAttributes(2.f, LineAttributes.PATTERN_SOLID, true);
        // appearance.setLineAttributes(lineAttributes);
        appearance.setColoringAttributes(new ColoringAttributes(1.f, 0.f, 0.f,ColoringAttributes.SHADE_FLAT));
      ...
      if (!true) { // y axis
        ...
        LineArray yla = new  LineArray(vcnt, GeometryArray.COORDINATES | GeometryArray.COLOR_3);
        ...
        Appearance appearance = new Appearance();
        appearance.setColoringAttributes(new ColoringAttributes(0.f, 1.f, 0.f,ColoringAttributes.SHADE_FLAT));
      ...
      if (!true) { // z axis
        ...
        LineArray zla = new  LineArray(vcnt, GeometryArray.COORDINATES | GeometryArray.COLOR_3);
        ...
        Appearance appearance = new Appearance();
        appearance.setColoringAttributes(new ColoringAttributes(0.f, 0.f, 1.f,ColoringAttributes.SHADE_FLAT));
      ...
      if (!true) {
        ...
        QuadArray qa = new QuadArray(coords.length, GeometryArray.COORDINATES|GeometryArray.NORMALS);
        qa.setCoordinates(0,coords);
        qa.setNormals(0,normals);
        Appearance appearance = new Appearance();
        appearance.setTransparencyAttributes(new TransparencyAttributes(TransparencyAttributes.FASTEST, .99f));
        Shape3D shape = new Shape3D(qa, appearance);
        axesGroup.addChild(shape);
      }
  ...
  public void setData() {
    ...
    dataMaps.setCapability(Switch.ALLOW_CHILDREN_READ);
    dataMaps.setCapability(Switch.ALLOW_CHILDREN_WRITE);
    dataMaps.setCapability(Switch.ALLOW_CHILDREN_EXTEND);


    dataGroup = new BranchGroup();
    dataGroup.setCapability(BranchGroup.ALLOW_DETACH);
    dataGroup.setCapability(BranchGroup.ALLOW_BOUNDS_READ);
    dataGroup.setCapability(BranchGroup.ALLOW_CHILDREN_READ);
    dataGroup.setCapability(BranchGroup.ALLOW_CHILDREN_WRITE);
    dataGroup.setCapability(BranchGroup.ALLOW_CHILDREN_EXTEND);
