/*
 * @(#) $RCSfile: DataViewer.java,v $ $Revision: 1.1 $ $Date: 2003/05/15 18:23:49 $ $Name: TableView1_2 $
 *
...
public class DataViewer extends JPanel implements Serializable {
  ...
  DataViewer() {
   ...
   if (true) {
    dataMapJList.setSelectionMode( ListSelectionModel.MULTIPLE_INTERVAL_SELECTION);
...
/*
    ...
    gbl.setConstraints(ctrls, c);
    add(ctrls);
    c.gridheight = GridBagConstraints.REMAINDER;
    c.weighty = 1.0;
    c.fill = GridBagConstraints.BOTH;
    gbl.setConstraints(dv, c);
    add(dv);
*/
  ...
  /** Import a data set into this view.
    * @param source The data source location (file pathname or URL).
    */
  ...
  class Controls extends JPanel {
    ...
    public void addDataMap(DataMap ds) {
      JMenuItem mi = (JMenuItem)dataMapMenu.add(new JCheckBoxMenuItem(ds.getName()));
...
System.err.println("adding to menu: " + ds.getName());
    ...
    Controls() {
      ...
      fileMenu = new JMenu("File");
        mi = (JMenuItem)fileMenu.add(new JMenuItem("import data..."));
        mi.setMnemonic('i');
...
*/
      ...
      JButton dataCtrl = new JButton("Data Maps");
        ...
        dataCtrl.getAccessibleContext().setAccessibleDescription("Data Maps");
...
if(true) {
      ...
      for (int i = 0; i < dataReps.length; i++) {
        mi = (JMenuItem)repMenu.add(
            new JRadioButtonMenuItem(dataReps[i], i==dataReps.length-1));
