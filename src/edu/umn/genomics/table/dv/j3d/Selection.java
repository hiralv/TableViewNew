/*
 * @(#) $RCSfile: Selection.java,v $ $Revision: 1.2 $ $Date: 2003/07/28 15:07:19 $ $Name: TableView1_2 $
 *
 ...
 * sets the cooresponding indices in a ListSelectionModel.  The user drags 
 * out a circular region that has a diameter from the mouse pressed point 
 * to the mouse release point.  The set of selected points are applied 
 * to current selected set using the set operator from the SetOperator.
 * 
 * This class relies on a PickCanvas that is available in Java3D1.2.
 * @author       J Johnson
 * @version $Revision: 1.2 $ $Date: 2003/07/28 15:07:19 $  $Name: TableView1_2 $ 
 * @since        1.0
...
public class Selection extends Behavior implements Serializable {
   ...
   * canvas.
   * @param canvas3D The canvas on which the selection will be performed.
   * @param branchGroup The BranchGroup that contains the geometry to be picked.
   * @param size The default minimum radius in pixels for picking points.
   * @param lsm The model in which to record the point indices picked.
   * @param setOperator provides the set operator 
   */
  public Selection(Canvas3D canvas3D, BranchGroup branchGroup, 
		float size, ListSelectionModel lsm, SetOperator setOperator) {
	...
	for (int i=0; i<event.length; i++) { 
	  ...
	  if (id == MouseEvent.MOUSE_PRESSED) {
            ...
            if (setOperator != null) {
              ...
              setOperator.setFromInputEventMask(((MouseEvent)event[i]).getModifiers());
            }
	    ...
	    int cy = ((MouseEvent)event[i]).getY();
            int lx = lastPoint.x;
            int ly = lastPoint.y;
            double r = Math.sqrt(((cx-lx)*(cx-lx)) + ((cy-ly)*(cy-ly))) * .5;
	    ...
	    */
		...
		GeometryArray curGeomArray = (GeometryArray) curGeom;
		  ...
		  PickIntersection pi = 
		    pickResult[j].getClosestIntersection (eyePos);
		  ...
		  if (coordidx != null && coordidx.length > 0) {
		    ...
		    System.out.print("Indices: " + (lsm != null));
                    for (int k=0;k<coordidx.length;k++) {
                      if (lsm != null)
                      lsm.addSelectionInterval(coordidx[k], coordidx[k]);
                      if (k % 6 == 0) 
                        System.out.print("\n\t");
		  ...
		  if (iNormal != null) {
		    System.out.println ("Interpolated normal: "+iNormal);
		      ...
		      PickIntersection pi = pickResult[j].getIntersection(k);
                      int []coordidx = pi.getPrimitiveCoordinateIndices();
                      if (coordidx != null && coordidx.length > 0) {
                        for (int l=0;l<coordidx.length;l++) {
                          lsm.addSelectionInterval(coordidx[l], coordidx[l]);
                        }
                      }
                    }
                    break;
		      ...
		      PickIntersection pi = pickResult[j].getIntersection(k);
                      ...
                      if (coordidx != null && coordidx.length > 0) {
                        for (int l=0;l<coordidx.length;l++) {
                          lsm.removeSelectionInterval(coordidx[l],coordidx[l]);
                        }
                      }
                    }
                    break;
