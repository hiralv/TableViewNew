/*
 * @(#) $RCSfile: DataMapJ3D.java,v $ $Revision: 1.1 $ $Date: 2003/05/15 18:23:48 $ $Name: TableView1_2 $
 *
 ...
 * displayable attributes in java3D.
 * It creates a single Java3d BranchGroup that can be added to a scene to 
 * represent the data values of the TableModel.
 * It overides the TableModelListener method tableChanged so that it can 
 * modify the BranchGroup in response to data changes.
 * @author       James E Johnson
 * @version $Revision: 1.1 $ $Date: 2003/05/15 18:23:48 $  $Name: TableView1_2 $ 
 * @since dv1.0
 */
public class DataMapJ3D extends DataMap implements Serializable {
   ...
   * @param name A name to identify this data mapping.
   * @param tableModel The table to map to displayable attributes.
   * @param lsm The ListSelectionModel for selecting elements of the table.
   */
  DataMapJ3D(String name, TableModel tableModel, ListSelectionModel lsm) {
    ...
    pRep.setCapability(BranchGroup.ALLOW_BOUNDS_READ);
    pRep.setCapability(BranchGroup.ALLOW_CHILDREN_READ);
    pRep.setCapability(BranchGroup.ALLOW_CHILDREN_WRITE);
    pRep.setCapability(BranchGroup.ALLOW_CHILDREN_EXTEND);

    gRep = new BranchGroup();
    // Assign the capabilities we need.
    gRep.setCapability(BranchGroup.ALLOW_DETACH);
    gRep.setCapability(BranchGroup.ALLOW_BOUNDS_READ);
    gRep.setCapability(BranchGroup.ALLOW_CHILDREN_READ);
    gRep.setCapability(BranchGroup.ALLOW_CHILDREN_WRITE);
    gRep.setCapability(BranchGroup.ALLOW_CHILDREN_EXTEND);
    ...
    dsGroup.setCapability(BranchGroup.ALLOW_BOUNDS_READ);
   ...
   * @param tableModelEvent describes changes to the TableModel.
   */
...
//System.err.println(name + " table changed " + first + " " + last);
   ...
   * @param scale the scaling factor for the data point glyphs.
   */
   ...
   * @param scale the relative scaling factor for the data point glyphs.
   */
    ...
    if (canvas != null) {
      try {
        if (Class.forName("com.sun.j3d.utils.picking.PickTool") != null) {
          selector = new Selection(canvas, pGroup, 0.05f, lsm, setOperator);
  ...
  private void addAll() {
    if (whichChild == 0) {
      ...
      pGroup.setCapability(BranchGroup.ALLOW_BOUNDS_READ);
      pGroup.setCapability(BranchGroup.ALLOW_CHILDREN_READ);
      pGroup.setCapability(BranchGroup.ALLOW_CHILDREN_WRITE);
      pGroup.setCapability(BranchGroup.ALLOW_CHILDREN_EXTEND);
      makePoints();
      ...
      gGroup.setCapability(BranchGroup.ALLOW_BOUNDS_READ);
      gGroup.setCapability(BranchGroup.ALLOW_CHILDREN_READ);
      gGroup.setCapability(BranchGroup.ALLOW_CHILDREN_WRITE);
      gGroup.setCapability(BranchGroup.ALLOW_CHILDREN_EXTEND);
   ...
   * @param i The row index in the TableModel for this data point.
   */
   ...
   * @param i The row index in the TableModel
   */
   ...
   * @param i The row index in the TableModel for this data point.
   * @param isSelected The selection status of this data point.
   */
  public void selectDataPoint(int i, boolean isSelected) {
    ...
    if (pGroup != null) {
/*
      Color col = isSelected ? selectColor : getPointColor(i);
      Color3f c3f = new Color3f(col.getColorComponents((float[])null));
      pa.setColor(i,c3f);
*/
    }
  }
  ...
  private void makePoints() {
    ...
    if (rows > 0) {
      ...
      pa.setCoordinates(0, getPointTranslation(0, rows, (float[])null, 0, 3));
  ...
  private void _makeSelectPoints(ListSelectionEvent e) {
    ...
    if (rows > 0) {
      ...
      if (rows < 1) 
        return;
      boolean useColor = colormapIndex >= 0 && colorMap != null;
      int vertexFormat = GeometryArray.COORDINATES|GeometryArray.COLOR_4;
      ...
      sGroup.setCapability(BranchGroup.ALLOW_BOUNDS_READ);
      sGroup.setCapability(BranchGroup.ALLOW_CHILDREN_READ);
      sGroup.setCapability(BranchGroup.ALLOW_CHILDREN_WRITE);
      sGroup.setCapability(BranchGroup.ALLOW_CHILDREN_EXTEND);
...
/*
  public void setPositionSource(int column[]) {
    super.setPositionSource(column);
  }
