/*
 * @(#) $RCSfile: CollectionTableModel.java,v $ $Revision: 1.4 $ $Date: 2002/07/30 19:45:01 $ $Name: TableView1_3 $
 *
 ...
 * Collection.  The Collection itself, or fields and methods of 
 * the most specific common Class for all members of the Collection, 
 * may be used as columns of the TableModel.
 * @author       J Johnson
 * @version $Revision: 1.4 $ $Date: 2002/07/30 19:45:01 $  $Name: TableView1_3 $ 
 * @since        1.0
...
public class CollectionTableModel extends AbstractTableModel 
  ...
  /** Columns defined for the TableModel. 
   *  A column will be a Method, a Field, or the collection itself.
   */
  LinkedList column = new LinkedList();

  /** 
   * Create a TableModel interface for the given collection.
   * The collection is copied, so changes to the given collection 
   * won't be reflected in the TableModel.
   * @param collection The Collection to be presented as a TableModel.
   */
   ...
   * @param m A method of the common class of the collection.
   * @see #getCommonClass()
   */
   ...
   * @param f A field of the common class of the collection.
   * @see #getCommonClass()
   */
  ...
  /**
   * Remove the given field or method from the columns of the TableModel.
   * This may alter the indices of remaining columns.
   * @param o A field or method that was previously added as a column.
   * @see #addColumn()
   * @see #addColumn(Field)
   ...
   * This may alter the indices of remaining columns.
   * @param index The index of a column of the TableModel..
   * @see #addColumn()
   * @see #addColumn(Field)
   * @see #addColumn(Method)
   */
   ...
   * 
   */
  public Object[] getMethodArgs(Method m, Object o, 
  ...
  public Object getValueAt(int rowIndex, int columnIndex) {
    if (collection != null && rowIndex >= 0 && rowIndex < collection.size() &&
        column != null && columnIndex >= 0 && columnIndex < column.size()) {
      try {
        ...
        if (co instanceof Field) {
          ...
          Method m = (Method)co;
//System.err.println(m.getName() + " " + m.getParameterTypes().length);
          return m.invoke(o,getMethodArgs(m, o, rowIndex, columnIndex));
        } else if (co instanceof TableModelFormula) {
          return ((TableModelFormula)co).getValueAt(rowIndex, columnIndex);
        } else {
          return o;
        }
      } catch (Exception e) {
