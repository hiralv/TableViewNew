/*
 * @(#) $RCSfile: ClusterView.java,v $ $Revision: 1.2 $ $Date: 2003/05/28 17:44:53 $ $Name: TableView1_2 $
 *
...
import edu.umn.genomics.graph.GraphDataModel;
import edu.umn.genomics.graph.GraphSegments;
import edu.umn.genomics.graph.LinearAxis;
import edu.umn.genomics.graph.swing.IndexSelectColor;
...
/**
 * ClusterView displays a hierarchical clustering of rows from a table.
 ...
 * @author       J Johnson
 * @version $Revision: 1.2 $ $Date: 2003/05/28 17:44:53 $  $Name: TableView1_2 $ 
 * @since        1.0
 * @see  ColumnMap
 * @see  TableContext
 ...
 */
public class ClusterView extends AbstractTableModelView
  ...
  /** 
   * Make selections on the graph panel.
   * User draws out a rectangle on the graph.
   * Any nodes intersected by the rect will be selected.
   * Note that this sets the ListSelection model for the table rows,
   * but it is the listener of the TableModel row ListSelectionModel 
   * that will actually set the selection of the line segments for 
  ...
  private MouseAdapter ma = new MouseAdapter() {
    ...
    public void mouseReleased(MouseEvent e) {
      ...
      Rectangle selrect = new Rectangle(start.x,start.y,current.x-start.x,current.y-start.y);
      ...
      repaint();
        ...
        public void mouseClicked(MouseEvent e) {
            if (e.getClickCount() >= 1) {
                Rectangle selrect = new Rectangle(e.getX() - 1, 0, 1, graph.getGraphDisplay().getHeight());
                int[] gi = gs.getIndicesAt(selrect, graph.getXAxis(), graph.getYAxis());
                System.err.println("partitions " + (gi != null ? gi.length : 0));
                if (e.getClickCount() == 2) {
                    int[] pi = new int[tm.getRowCount()];
                    ...
                    for (TreeNode tn : tns) {
                        selectTraverse(tn, pi, tns.indexOf(tn));
                    }
                    PartitionIndexMap pim = new PartitionIndexMap(pi);
                    Partition partition = new DefaultPartition(getName(), pim, null);
                    TableModel ptm = getTableContext().getTableModel(tm, partition);
                }
            }
        }
    };
...
//        public void mouseClicked(MouseEvent e) {
//            if (e.getClickCount() == 1) {
//                Rectangle selrect = new Rectangle(e.getX(), 0, 1, graph.getGraphDisplay().getHeight());
//                int[] gi = gs.getIndicesAt(selrect, graph.getXAxis(), graph.getYAxis());
//                System.err.println("partitions " + (gi != null ? gi.length : 0));
//                
//                for (int j = 0; j < gi.length; j++) {
...
//                    int nodeidx = gi[j] / 2;
//                    TreeNode tn = nodemap[nodeidx];
//                    DefaultListSelectionModel psm = new DefaultListSelectionModel();
//                    selectTraverse(tn, psm);
//                    System.err.println("partition " + nodeidx + " " + psm.getMinSelectionIndex() + "-" + psm.getMaxSelectionIndex() + " (" + tn + ") ");
//                }
...
//                partitionLine = new GraphSegments();
//                partitionLine.setColor(Color.DARK_GRAY);
//                graph.addGraphItem(partitionLine);
//            }
//            double[] endPoints = new double[4];
////            endPoints[0] = endPoints[2] = ((AxisDisplay) e.getSource()).getValueAt(e.getPoint());            
//            endPoints[0] = endPoints[2] = graph.getAxisDisplay(BorderLayout.NORTH).getValueAt(e.getPoint());            
//            endPoints[1] = graph.getAxisDisplay(BorderLayout.EAST).getAxis().getMin();
//            endPoints[3] = graph.getAxisDisplay(BorderLayout.EAST).getAxis().getMax();
//            partitionLine.setData(endPoints);
//            graph.repaint();
//        }
//
//    };
  ...
  /**
   * Constructs a ClusterView display.  Nothing will be displayed
...
//    graph.getAxisDisplay(BorderLayout.NORTH).addMouseListener(pma);
//    graph.getAxisDisplay(BorderLayout.NORTH).addMouseMotionListener(pma);
  ...
  /**
   * Recursive traverse of tree to determine selections
   * A leaf is selected if rsm is selected.
   ...
   */
  private boolean selTraverse(TreeNode tn, int[] nodeidx, ListSelectionModel gsm,
  ...
  /**
   * Traverse the tree selecting rows corresponding to leaf nodes of 
   * the given node tn
   ...
   * @param rsm the selection model in which to mark selected rows
   */
  private void selectTraverse(TreeNode tn, int[] rowIndex, int label) {
    if (!tn.isLeaf()) {
      for ( int i = 0; i < tn.getChildCount(); i++) {
        TreeNode cn = tn.getChildAt(i);
        selectTraverse(cn, rowIndex, label);
      }
    } else {
      if (tn instanceof RowCluster) {
        int ri = ((RowCluster)tn).getIndex();
        if (rowIndex != null && ri < rowIndex.length) {
            rowIndex[ri] = label;
        }
      }
    }
  ...
  * a nodemap, and a leafmap.
  * Traversal is depth first.
  * return child position currentleafcnt 
...
*/
  ...
  /** 
   * Generate a list of line segments that construct a dendogram 
   * of the tree rooted at node tn.
