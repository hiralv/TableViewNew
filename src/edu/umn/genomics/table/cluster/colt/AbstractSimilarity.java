/*
 * @(#) $RCSfile: AbstractSimilarity.java,v $ $Revision: 1.2 $ $Date: 2003/07/28 15:07:18 $ $Name: TableView1_2 $
 *
 ...
 * Extending classes need to define the distance calculation.
 * NOTE:  This currently doesn't handle null values in table cells.
 * @author       J Johnson
 * @version $Revision: 1.2 $ $Date: 2003/07/28 15:07:18 $  $Name: TableView1_2 $ 
 * @since        1.0
 */
public abstract class AbstractSimilarity implements ClusterSimilarity {
  static String distFunctions[] = {"EUCLID", "MANHATTAN", "MAXIMUM", "BRAY_CURTIS", "CANBERRA"};
  public int debug = 0;
   ...
   * @param distanceFunction The name of the distance function.
   */
   ...
   */
  public void setDistanceFunction(Statistic.VectorVectorFunction distanceFunction) {
  ...
  /**
   * Generate a distance matrix for the table, 
   * based on the values in the given columns.
   * @param ctx The context from which to get ColumnMaps.
   * @param tm The table from  which to get data values.
   * @param columns the column indexes to use for clustering.
   * @return the distance matrix 
   */
  public DoubleMatrix2D getDistMat(TableContext ctx, TableModel tm, int columns[]) {
   ...
   */
  public DoubleMatrix2D getDistMat(TableContext ctx, TableModel tm, int columns[],
                                   Statistic.VectorVectorFunction distanceFunc) {
      ...
      for (int c = r+1; c < distMat.rows(); c++) {
        ...
        if (Double.isNaN(dv)) {
          ...
          for (int i = 0; i < rm.size(); i++) {
            if (!Double.isNaN(rm.get(i)) && !Double.isNaN(cm.get(i))) {
              nonNaNs++;
            }
          }
          if (nonNaNs > 0) {
            DenseDoubleMatrix2D sm = new DenseDoubleMatrix2D(nonNaNs,2);
            ...
            DoubleMatrix2D dm = Statistic.distance(sm, distanceFunc);
  ...
  /**
   * Generate a tree that represents a hierarchical clustering of the rows 
   * of the table, based on the values in the given columns.
   * @param ctx The context from which to get ColumnMaps.
   * @param tm The table from  which to get data values.
   * @param columns the column indexes to use for clustering.
   * @return the root node of a tree that represents a hierarchical 
   *         clustering of the rows of the table.
   */
  ...
  /**
   * Generate a tree that represents a hierarchical clustering of the list 
   * of RowClusters.
   * @param items The RowClusters to cluster.
   * @param dm The distance matrix for the table values.
   * @return the root node of a tree that represents a hierarchical 
   *         clustering of the rows of the table.
   */
  protected TreeNode cluster(List items, DoubleMatrix2D dm) {
    ...
    CompositeCluster pn = null;		// The parent node for the merged rMin and cMin
    ...
    while (clusters.size() > 1) {
      ...
      if (nearest == null) {  // nearest is null, so we need to initialize
        ...
        nearest.setSize(n);	// Initialize the size so we can just use set method;
        da = new double[n];       // array of distances to nearest node
        // Initially compare each cluster with all the others after it in the list
        // record the cluster with the least distance in the neastest list, 
        //   and put the distance in the da distance array
        for (int r = 0; r < n-1; r++) { // loop until next to last node
          int rcmin = 0;
          double rdist = Double.MAX_VALUE;
          // since pairwise distance are transitive, we only need to 
          // compare to the remaining nodes
          for (int c = r+1; c < n; c++) { 
            if (debug > 4) System.err.print("\rr = " + r + "\t c = " + c);
            double tmp = distance((Cluster)clusters.get(r),(Cluster)clusters.get(c),dm);
            if (Double.isNaN(tmp)) {
              System.err.println("\tdistance NaN\t" + r + "\t" + c + "\t");
            }
            if (debug > 2) System.err.println("\tcluster comp \t" + r + "\t" + c + "\t" + tmp);
            if (tmp < rdist) {
              rdist = tmp;
              rcmin = c;
              if (debug > 1) System.err.println(r + "\tclusters found \t" + r + "\t" + rcmin + "\t" + rdist);
            }
          }
          if (rcmin > 0) {
            ...
            nearest.set(r,nn);
            da[r] = rdist;
if (debug > 0) System.err.println(" " + r + "\tinitial" + "\t" + rcmin + "\t\t" + rdist);
            if (rdist < dist) { // remember the nearest pair to merge next
              dist = rdist;
              rmin = r;
              cmin = rcmin;
            }
          } else {
            System.err.println("Cluster error " + r + " " + clusters.get(r));
          }
        }
        ...
        if (debug > 2) {
          for (int r = 0; r < clusters.size(); r ++) {
            ...
            System.err.println(" > " + r + "\t" + + clusters.indexOf(nc) + "\t" + da[r]);
          }
        }
        // loop through the list and find new nearest 
        for (int r = 0; r < clusters.size()-1; r ++) {
          Cluster nc = (Cluster)nearest.get(r);
          // if nearest is null or nearest is either of the last nodes we merged
          if (nc == null || nc == cMin || nc == rMin) {
            int rcmin = 0;
            double rdist = Double.MAX_VALUE;
            // Search through the entire remining list looking for the new nearest
            ...
            for (int c = r+1; c < n; c++) {
              ...
              if (tmp < rdist) { // this one is now the nearest
                rdist = tmp;
                rcmin = c;
                if (debug > 1) System.err.println("\rclusters found \t" + rmin + "\t" + rcmin + "\t" + rdist)
    ;
              }
            }
            if (rcmin > 0) {
              Cluster nn = (Cluster)clusters.get(rcmin);
              nearest.set(r,nn);
if (debug > 0) System.err.println(" " + r + "\t recalc " + "\t" + rcmin + "\t" + (nc == cMin ? "c "+lcmin : nc == rMin ? "r " + lrmin : "null") + "\t" + rdist + "\t" + da[r]);
              da[r] = rdist;
            } else {
              System.err.println("Cluster error " + r + " " + clusters.get(r));
            }
          } else { // we only need to check the distance to the new parent node
            double tmp = distance((Cluster)clusters.get(r),pn,dm);
            if (Double.isNaN(tmp)) {
              System.err.println("\tdistance NaN\t" + r + "\t" + (clusters.size()-1) + "\t");
            }
            if (tmp < da[r]) {
              // The parent node is now the nearest
if (debug > 0) System.err.println(" " + r + "\t pn     " + "\t" + (clusters.size()-1) + "\t" + clusters.indexOf(nc) + "\t" +  tmp + "\t" + da[r]);
              da[r] = tmp;
              nearest.set(r,pn);
            } 
          }
          if (da[r] < dist) { // remember the nearest pair to merge next
            rmin = r;
      ...
      if (cmin < 0) {
        System.err.println(clusters.size() + "\t" + rmin + "\t" + cmin + "\t" + dist + "\n" + rMin + "\n" + cMin );
      }
      ...
      if (debug > 0) System.err.println(clusters.size() + "\tclusters merge \t" + rmin + "\t" + cmin + "\t" + dist);

      // merge: create parent, delete children from clusters, add parent to clusters
      pn = new CompositeCluster(rMin,cMin);
      pn.setSimilarity(dist);
        try {
            int rLeafCnt = rMin.getLeafCount();
            int cLeafCnt = cMin.getLeafCount();
            double rf = (double) rLeafCnt / (rLeafCnt + cLeafCnt);
            double cf = (double) cLeafCnt / (rLeafCnt + cLeafCnt);
            double[] rv = rMin.getValues();
            double[] cv = cMin.getValues();
            double[] pv = new double[rv.length];
            for (int i = 0; i < pv.length; i++) {
                pv[i] = rf * rv[i] + cf * cv[i];
            }
            pn.setValues(pv);
        } catch (Exception e) {
      ...
      if (nearest.size() - cmin > 1)
         System.arraycopy(da,cmin+1, da, cmin - 1, nearest.size() - cmin - 1);

      // Remove merged nodes from the nearest list, then add null entry for parent
      if (cmin < nearest.size())
        nearest.remove(cmin);
      else
        System.err.println("nearest.remove " + cmin + " from " +  nearest.size());
      if (rmin < nearest.size())
        nearest.remove(rmin);
      nearest.add(null);  // need a null entry to indicate that we need to search 
   ...
   * The extending classes determine how to accumulate the leaf distances.
   * @param c1 The first cluster of the pair.
   * @param c2 The second cluster of the pair.
   * @param dm The distance matrix for RowClusters.
   * return the distance between the clusters.
   */
  protected abstract double distance(CompositeCluster c1, Cluster c2, DoubleMatrix2D dm);
