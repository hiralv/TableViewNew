/*
 * @(#) $RCSfile: ClutoGraphView.java,v $ $Revision: 1.1 $ $Date: 2003/07/28 19:30:19 $ $Name: TableView1_2 $
 *
 ...
 * @author       J Johnson
 * @version $Revision: 1.1 $ $Date: 2003/07/28 19:30:19 $  $Name: TableView1_2 $ 
 * @since        1.0
 * @see  ColumnMap
 * @see  TableContext
...
public class ClutoGraphView extends JPanel implements CleanUp, Serializable {
  ...
  /**
   * A view of the ClutoSolution that displays the rows of the matrix as line graphs 
   * for each cluster.
   * 
   */
  public ClutoGraphView(ClutoSolution clutoSolution, PartitionIndexMap partMap, ListSelectionModel lsm) {
    ClutoTableMatrix cMat = clutoSolution.getMatrix() instanceof ClutoTableMatrix 
                            ? (ClutoTableMatrix)clutoSolution.getMatrix()
                            : new ClutoTableMatrix(clutoSolution.getMatrix());

    
    ClusterTableModel tm = new ClusterTableModel(clutoSolution.getParams().getNumClusters()); 
    jtable = new JTable(tm);
    jtable.getColumnModel().getColumn(0).setPreferredWidth(30);
    // jtable.getColumnModel().getColumn(1).setPreferredWidth(Math.max(cMat.getRowCount(),200));
    ...
    ClusterGraph[] cg = new ClusterGraph[clutoSolution.getParams().getNumClusters()];
    ...
    for (int c = 0; c < cg.length; c++) {
      ...
      if (c > 0) {
        ...
        if (shareAxis) cg[c].getGraph().setYAxis(cg[0].getGraph().getYAxis());
      }
      ...
      gpP.add(cg[c]);
      AxisDisplay yDisp = new AxisDisplay(cg[0].getGraph().getYAxis(), AxisComponent.LEFT);
    ...
    ComponentAdapter jta = new ComponentAdapter() {
      public void componentResized(ComponentEvent e) {
        jtable.setRowHeight(((JComponent)e.getSource()).getHeight()+2);
      }
    };
    ...
    ((LinearAxis)(cg[cg.length-1].getGraph().getXAxis())).setTickIncrement(1.);
    Axis xAxis = new LinearAxis(cg[0].getGraph().getXAxis());
    AxisDisplay xDisp = new AxisDisplay(cg[0].getGraph().getXAxis(), AxisComponent.TOP);
    ...
    try {
      ...
      ChangeListener changeListener = new ChangeListener () {
          public void stateChanged(ChangeEvent e) {
            try {
              double scale = ((Number)DoSpinner.getValue(e.getSource())).doubleValue();
