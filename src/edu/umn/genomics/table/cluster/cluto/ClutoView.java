/*
 * @(#) $RCSfile: ClutoView.java,v $ $Revision: 1.2 $ $Date: 2003/10/17 16:16:23 $ $Name: TableView1_2 $
 *
 ...
 * @author       J Johnson
 * @author       Shulan Tian
 * @version $Revision: 1.2 $ $Date: 2003/10/17 16:16:23 $  $Name: TableView1_2 $ 
 * @since        1.0
 * @see  ColumnMap
 * @see  TableContext
...
public class ClutoView extends AbstractTableModelView
  ...
  final ClutoOptionsPanel clutoOptions = new ClutoOptionsPanel(ClutoParams.VP_ClusterRB);
  ...
  private void addClutoSolution() {
    ...
    SolutionPanel solPanel = new SolutionPanel(cMat, getTransParams(), getClutoParams(), colList.getSelectedIndices());
    // add it to solTP
    solTP.add(solPanel);
  ...
  private void clearClutoSolution() {
    ...
    clutoOptions.setMatrixSize(tm.getRowCount(), colList.getSelectedValues().length);
  }
   ...
   */
  private JComponent getSolutionViews(ClutoSolution clutoSolution, TransParams tParams, int[] listIndex) {
    ...
    viewTabs.add("Tree", 
                 getFloatingComponent(clutoSolution,
                                      ...
                                      "Tree"));
    // Matrix
    viewTabs.add("LogRatio Tree Matrix", 
                 getFloatingComponent(clutoSolution,
                                      getLogRatioView(clutoSolution),
   ...
   * @param clutoSolution The cluto solution that is represents
   ...
   */
  private JComponent getSolutionSummary(ClutoSolution clutoSolution, TransParams tParams, int[] listIndex) {
    ...
    if (tParams != null) {
      ...
      sb.append("  Missing_Value_Estimation: ").append(ClutoResource.getParamValueName("MISSING_VALUE_ESTIMATION",tParams.getEstMethod()));
      if (tParams.getEstMethod() == TransParams.KNN) {
        sb.append("  #Neighbors: ").append(tParams.getNumNeighbors());
      }
      if (tParams.getLogTrans()) {
        sb.append(",  ").append(ClutoResource.getName("MATRIX_TRANSFORM_LOG"));
      }
      if (tParams.getMeanCenter()) {
        sb.append(",  ").append(ClutoResource.getName("MATRIX_TRANSFORM_MEAN_CENTERED"));
      }
      if (tParams.getMedianCenter()) {
        sb.append(",  ").append(ClutoResource.getName("MATRIX_TRANSFORM_MEDIAN_CENTERED"));
      }
      sb.append("\n\n");
    }
    /* This is how the Cluto Options are reported in the commandline application vcluster 
    ...
    */
    ...
    sb.append("  CLMethod=").append(ClutoResource.getParamValueName("CLUTO_CLUSTER_METHOD",cp.getClusterMethod())).append(",");
    sb.append("  CRfun=").append(ClutoResource.getParamValueName("CLUTO_CLFUN",cp.getCrFunc())).append(",");
    sb.append("  SimFun=").append(ClutoResource.getParamValueName("CLUTO_SIM",cp.getSimFunc())).append(",");
    sb.append("  #Clusters: ").append(cp.getNumClusters()).append("\n");
    sb.append("  RowModel=").append(ClutoResource.getParamValueName("CLUTO_ROWMODEL",cp.getRowModel())).append(",");
    sb.append("  ColModel=").append(ClutoResource.getParamValueName("CLUTO_COLMODEL",cp.getColModel())).append(",");
    sb.append("  Colprune=").append(cp.getColPrune()).append("\n");
    sb.append("  CSType=").append(ClutoResource.getParamValueName("CLUTO_CSTYPE",cp.getCsType())).append(",");
    sb.append("  AggloFrom=").append(cp.getAggloFrom()).append(",");
    sb.append("  AggloCRFun=").append(ClutoResource.getParamValueName("CLUTO_CLFUN",cp.getAggloCrFunc())).append(",");
    ...
    sb.append(" " ).append("=").append(clutoSolution.getSolutionQuality()).append("]").append("\n");
    sb.append("\n");
    ...
    for (int r = 0; r < stm.getRowCount(); r++) {
      for (int c = 0; c < stm.getColumnCount(); c++) {
        sb.append("\t").append(df.format(((Number)stm.getValueAt(r,c)).floatValue()));
      }
      sb.append("\n");
    }
    sb.append("\n");
    ...
    if (clutoSolution instanceof ClutoCluster) {
      ...
      sb.append("\tclustering\t").append( ((ClutoCluster)clutoSolution).getClusterTime()).append(" secs\n");
    }
  ...
  private JComponent getMatrixView(ClutoSolution clutoSolution) {
    ClutoTableMatrix cMat = new ClutoTableMatrix(clutoSolution.getInternalMatrix());
    FontMetrics fm = null;
    try {
      fm = this.getGraphics().getFontMetrics();
    } catch (Exception ex) {
  ...
  private void init() {
    ...
    listPane.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(),
                                                  "Select Columns"));
    left.add(listPane);
    ...
    if (comp instanceof JTabbedPane) {
      ...
      if (c != null && c instanceof JComponent) {
        if (c instanceof JComponent) {
          ...
          if (jc.getComponentCount() > 0 && jc.getComponent(0) instanceof TearOff) {
            TearOff toff = (TearOff)jc.getComponent(0); 
            c = toff.getComponentAtIndex(0);
            if (c instanceof ClutoMatrixView) {
              c = ((ClutoMatrixView)c).getComponent(0);
            } else if (c instanceof ClutoGraphView) {
              c = ((ClutoGraphView)c).getComponent(0);
            } else if (c instanceof JTableView) {
              c = ((JTableView)c).getCanvas();
            }
          }
        }
        return c;
