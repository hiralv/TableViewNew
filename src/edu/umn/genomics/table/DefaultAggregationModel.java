/*
 * @(#) $RCSfile: DefaultHistogramModel.java,v $ $Revision: 1.3 $ $Date: 2004/08/18 18:04:16 $ $Name: TableView1_3 $
 *
...
/**
 * BinModel.
 * AggregationModels for columns determine the partitioning of the table 
 * similarly to a GROUP BY clause in SQL.
 * Each column used to define the group will be used as a dimension in an index array
 * that would hold the value of an aggregate function such as count.
 * A resulting table will consist of the group by columns 
 * and any additional columns defined as an aggregate function on the groups.  
 * A column with the count of the items in each group can define a histogram.  
 * 
 * 
 * Aggregate functions that can be applied to any column: count, min, max, median
 * Aggregate functions that can be applied to Number columns: sum, avg, stddev, variance
 *    
 * 
 * @author       J Johnson
 * @version      %I%, %G%
 * @since        1.0
 */
public class DefaultAggregationModel extends AbstractHistogramModel implements TableModel,Runnable {
    public enum AggregateFunction {
        COUNT, MIN, MAX, MEDIAN, AVG, STDDEV, VARIANCE
    };
    class AggregateColumn<T> {
        
        int referenceTableColumn;
        AggregateFunction func;
        AggregateColumn(int referenceTableColumn, AggregateFunction func) {
   ...
   * Create
   */
  public DefaultAggregationModel() {
  }
   ...
   * @param lsm The selection model for the data source.
   */
...
//System.err.println("!!!! calculateBinSizes@" + Integer.toHexString(hashCode()) );
            return false;
          }
          if (dims[i] > 0) {
...
//System.err.println("<<<  calculateBinSizes@" + Integer.toHexString(hashCode()) );
  ...
  /**
   * Update the count foreach bin, notifying listeners 
   * before the calculation begins and after it is done.
   */
   ...
   * @param indices an array that locates a particular bin.
   ...
   * @param itemIndex The ordinal index of the item in the list or table.
   * @return The indices of the bin for the item with the given index.
   */
  ...
  /**
   * Select items referenced by this bin and perform a set operation 
   * with the items previously selected in the given listSelectModel.
   * @param indices an array that locates a particular bin.
   * @param listSelectModel The selection model to change with the bin selection, 
   *        if null a new ListSelectionModel will be allocated.
   * @param setOperator The Set operation used when applying the new selection.
   * @return The ListSelectionModel resulting from the selection, this will be the listSelectModel
   *         parameter if that was not null;
   */
  public ListSelectionModel selectBins(int[] indices, ListSelectionModel listSelectModel, int setOperator) {
    ListSelectionModel sm = listSelectModel != null ? listSelectModel : new DefaultListSelectionModel();
   ...
   *         parameter if that was not null;
   */
  public ListSelectionModel selectBin(int[] indices, ListSelectionModel listSelectModel, int setOperator) {
    singleSelectIndex[0] = MultiDimIntArray.getIndex(indices, getDimensions());
  ...
  /**
   * Select items referenced by this bin.
   * @param index locates a particular bin.
   */
   ...
   * @param indices an array that locates selected bins.
   */
   ...
   * @param indices The indices of the bin.
