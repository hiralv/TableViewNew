/*
 * @(#) $RCSfile: AbstractHistogramViewND.java,v $ $Revision: 1.1 $ $Date: 2004/08/02 20:23:37 $ $Name: TableView1_3 $
 *
 ...
 * of the table.
 * @author       J Johnson
 * @version $Revision: 1.1 $ $Date: 2004/08/02 20:23:37 $  $Name: TableView1_3 $ 
 * @since        1.0
 * @see  ColumnMap
 * @see  TableContext
...
public abstract class AbstractHistogramViewND extends AbstractTableModelView
 ...
 ActionListener quarterByYearListener = new ActionListener() {
  ...
  ActionListener colSetListener = new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        JMenuItem mi = (JMenuItem)e.getSource();
        String colName = mi.getText();
        int idx = Integer.parseInt(mi.getActionCommand());
  ...
  private void editBinModels() {
    if (editorDialog == null) {
      editorDialog = new BinModelEditorDialog((JFrame)this.getTopLevelAncestor(),hgm);
    }
...
System.err.println(">>>   updateColumns");
    if (tm != null) {
      ...
      if (needUpdate) {
        try {
          setMenu.removeAll(); 
          addMenu.removeAll(); 
          monthByYearMenu.removeAll();
          quarterByYearMenu.removeAll();
          int ncols = tm.getColumnCount();
          for (int c = 0; c < ncols; c++) {
            ...
            mi = setMenu.add(name);
            mi.setActionCommand(""+c);
            mi.addActionListener(colSetListener);
            ColumnMap cMap = ctx.getColumnMap(tm,c);
            if (cMap.isDate()) {
System.err.println("      updateColumns " + c + "\t" + name + "\t" + cMap.getName());
              // monthByYearMenu
...
System.err.println("<<<   updateColumns");
  ...
  public void setYearToYearMonthly(int column) {
    ...
    if (cmap.isDate()) {
      ...
      setDateView(cmap,Calendar.MONTH, null, cal.getTime(), Calendar.YEAR, 1);
    }
  }
  ...
  public void setYearToYearQuarterly(int column) {
    ...
    if (cmap.isDate()) {
      ...
      setDateView(cmap,Calendar.MONTH, labels, cal.getTime(), Calendar.YEAR, 1);
    }
  }

  public void setDateView(ColumnMap cmap, int calendarPartition, String[] labels, Date startDate, int calendarRange, int incr) {
    if (cmap.isDate()) {
      ColumnMapBinModel bmp = new ColumnMapBinModel(cmap);
      BinModelEditor.setDatePartition(bmp, cmap, calendarPartition, labels);
      ColumnMapBinModel bmr = new ColumnMapBinModel(cmap);
      BinModelEditor.setDateRange(bmr, cmap, startDate, calendarRange, incr);
      hgm.removeBinModels();
      hgm.addBinModel(bmp);
      hgm.addBinModel(bmr);
      // Update delMenu
