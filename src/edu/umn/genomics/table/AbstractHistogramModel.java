/*
 * @(#) $RCSfile: AbstractHistogramModel.java,v $ $Revision: 1.1 $ $Date: 2004/08/02 20:23:36 $ $Name: TableView1_3 $
 *
...
public abstract class AbstractHistogramModel implements HistogramModel {
   ...
   * @param index the index of BinModel.
   ...
   * @param model BinModel to remove.
   */
  ...
  /**
   * Return the number of selected items in the bin located by the given indices.
   * @param indices an array that locates a particular bin.
   ...
   */
  public abstract ListSelectionModel selectBin(int[] indices, ListSelectionModel listSelectModel, int setOperator); 
  ...
  /**
   * This is called whenever the BinModels have changed prior to 
   * firing a HistogramEvent.  This implementation does nothing, 
   * but the method may be overidden by extending classes.
   * @param e The event from a BinModel or null if this resulted from a change 
   *          in the collection of BinModels .
   */
  ...
  /**
   * Notify Listeners of a change.
   * @param isAdjusting Whether this model is still in the process of changing.
   */
  ...
  /**
   * Notify Listeners of a change.
   * @param removed an array of BinModels that have been removed
   * @param added an array of BinModels that have been added
   * @param isAdjusting Whether this model is still in the process of changing.
   */
  protected void fireHistogramEvent(BinModel[] removed, BinModel[] added, boolean isAdjusting) {
    ...
    for (int i = listeners.length-2; i>=0; i-=2) {
      if (listeners[i]==HistogramListener.class) {
        ...
        if (histogramEvent == null)
          histogramEvent = new HistogramEvent(this,removed,added,isAdjusting);
        ((HistogramListener)listeners[i+1]).histogramChanged(histogramEvent);
      }
    }
  }
}
