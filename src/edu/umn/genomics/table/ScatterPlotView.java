/*
 * @(#) $RCSfile: ScatterPlotView.java,v $ $Revision: 1.6 $ $Date: 2003/08/06 17:20:53 $ $Name: TableView1_3 $
 *
...
/**
 * ScatterPlotView displays a scatter plot of the two selected columns
 * of the table.
 * @author       J Johnson
 * @version $Revision: 1.6 $ $Date: 2003/08/06 17:20:53 $  $Name: TableView1_3 $ 
 * @since        1.0
 * @see  ColumnMap
 * @see  TableContext
 ...
 */
public class ScatterPlotView extends AbstractTableModelView
 ...
 */
    ...
    resbtn.addItemListener(new ItemListener() {
       public void itemStateChanged(ItemEvent e) {
         residuals = e.getStateChange() == ItemEvent.SELECTED;
       }
    });
    top.add(resbtn); 
/*
    ...
    if (ctx != null && plotPanel != null) {
      try {
        ...
        if (xMap != null && yMap != null ) {
          if (plot == null || (residuals && !(plot instanceof ResidualPlot)) 
                           || (!residuals && (plot instanceof ResidualPlot)) ) {
            ScatterPlot oldPlot = plot;
            plot = residuals ? new ResidualPlot(xMap, yMap) : new ScatterPlot(xMap, yMap);
            plot.setSelectionModel(ctx.getRowSelectionModel(tm));
            plot.getPlotComponent().addMouseListener(ma);
            if (oldPlot != null) {
              plotPanel.remove(oldPlot);
            }
            plotPanel.add(plot);
            validate();
          } else {
